...
### 44. The practical upper limit: perfect extent on each model's own sentence choice (2026-09-18 17:10)

Entry 42 gave the annotator-omniscient ceiling (perfect word range for every gold: tIoU 0.946, score 0.967). Entry 43 showed that about 8 % of positives are a two-statement ambiguity no model resolves. The ceiling that matters for planning keeps each model's *own* choice of utterance, ambiguity and all, and only perfects the extent: best word range under the served edge rule, restricted to the units the model cited (what clause-level units plus perfect trimming could reach) or to those units plus one neighbour on each side (what a perfect neighbour rule adds). Training set, turbo, accuracy held at the run's own value.

| run | mean tIoU now | perfect trim inside cited units | + one neighbour unit | zero-overlap floor |
|---|---:|---:|---:|---:|
| Qwen3.8-27B few-shot | 0.669 | 0.787 (score 0.868) | 0.834 (score 0.896) | 17 of 195 |
| Qwen3.8-27B joint-demo | 0.659 | 0.749 (0.847) | 0.837 (0.900) | 16 |
| Opus 5, 38 demos | 0.680 | 0.795 (0.877) | 0.848 (0.909) | 15 |
| Sonnet 5, 38 demos | 0.676 | 0.779 (0.866) | 0.834 (0.899) | 18 |
...
### 46. The 27B is now servable: model.py speaks to vLLM with the bench prompts, deadline and a local fallback (2026-09-18 12:20)

Until now the endpoint could only run the Ollama 4B with the plain `units` prompt; every stronger design existed only in the bench. model.py now takes `LLM_BACKEND=vllm` (an OpenAI-compatible `/v1` URL, schema-constrained JSON, thinking off through `chat_template_kwargs`) and `LLM_VARIANT=<name from bench/llm/prompts.py>` (default `units-fewshot` on vLLM, the design that scored 0.797 on training), so the served prompt is the benchmarked prompt byte for byte. The few-shot pool ships as `bench/llm/pool/large-v3-turbo.json` (390 examples, 195 positives, exported with `prompts.py --export-pool`), so a pod copy without the gitignored transcripts serves the same examples; the warm-up refuses to start with fewer than 150 positives. Timing: `LLM_DEADLINE` (40 s after the request arrived, ASR included) bounds each primary request to what is left minus a 12 s reserve; a transport failure opens a 20 s breaker so the other questions skip the dead server instead of each waiting out its timeout; anything that fails, times out or cannot be afforded goes to `LLM_FALLBACK_MODEL` (Ollama `qwen3:4b` on the laptop) with the same prompt, allowed to run 12 s past the deadline. Joint variants are supported (one request, per-question fallback with the plain units prompt). Tested on cached transcripts: ten questions in 7.2 s wall through Ollama's OpenAI layer with the 4B as a stand-in; with the primary pointed at a dead port, breaker plus fallback answered ten questions in 8.8 s. `bench/mine/send_dump.py` replays the dumped validation requests against any endpoint and `val_diag.py --dump` scores them, which is how the real pipeline gets its one held-out measurement once the pod serves. Cluster side: the DTU `bsub` does not pass the submitter's environment to the job (a submission with `MODELS=... bsub < llm_bench.lsf` ran with the defaults and skipped everything in ten seconds), so overrides go through a wrapper script that exports them; job 29442160 runs `units-joint-demo-fewshot` and `units-joint-demo-all` for both 27Bs on turbo transcripts, the two prompt designs the pod never finished last night.

### 47. Clause-level units: the unit-level oracle rises from 0.861 to 0.919; joint-demo-fewshot does not beat few-shot (2026-09-18 12:30)

**Joint-demo-fewshot on the 27B (DTU job 29442160): 0.782**, against few-shot 0.797 and joint-demo 0.794 (accuracy 0.997, mean tIoU 0.639; stored score equals the replay, so the `ASR_MODEL` fix holds on the cluster). Combining whole worked conversations with the nearest example lines costs span quality instead of adding to it. The served design stays `units-fewshot`.

**Clause units.** `model.make_units` takes a mode (`UNIT_SPLIT`, default `sentence`, byte-identical to the committed builder on all 97 cached transcripts). `clause` cuts a sentence at a comma or semicolon that starts a new clause (a conjunction follows, or a subject-verb clause of at least four words); `clause-all` cuts at every comma or semicolon; `clause-and` adds a cut before a comma-less coordinating conjunction (and, but, or, so). Pieces keep at least three words; a number is never separated from its unit. The rules are generic punctuation and conjunction rules, not fitted to individual golds. Unit-level oracle on the training golds (best contiguous run of up to six units under the served edge rule, turbo, `bench/units/clause_oracle.py`):

| unit mode | units per conversation | mean oracle tIoU | golds at 0.9 or above | score ceiling | gained 0.1 or more | lost 0.05 or more |
|---|---:|---:|---:|---:|---:|---:|
| sentence (served) | 59.1 | 0.861 | 128 of 195 | 0.916 | | |
| clause | 62.0 | 0.888 | 134 | 0.933 | 16 | 1 |
| clause-all | 63.4 | 0.904 | 139 | 0.942 | 24 | 1 |
...
What did not help at this size (every verdict below is for the named model and setup only; Elias's rule, 2026-09-18: an idea rejected at one model size stays a candidate at another, so these are the first cheap re-tests whenever the model changes). `clause-prep` (also cutting before comma-less prepositions and subordinators): oracle 0.921 against clause-and's 0.919 for five per cent more units; not benchmarked, removed. `units-fewshot-cl` (one prompt line explaining that a sentence can appear as several pieces, written after reading the 12 positives that lost tIoU, 9 of which cited fewer pieces than the gold covers): 0.8144 against 0.8125, +0.0019 ± 0.0046; the variant stays in `prompts.py` but the plain prompt is served. Many-shot on the 27B (`units-joint-demo-all`, 44.5k prompt tokens, 20.7 s per conversation): 0.7899, accuracy 390 of 390 but mean tIoU 0.650; on this model, as on Opus (entry 41) and unlike Sonnet (+0.027, entry 41), more worked conversations did not buy spans. Joint-demo-fewshot on the Qwen3.6-27B is still running in job 29442160 and no longer matters.
...
### 49. The 27B serves: pod, A100 reproduction, and the live end-to-end soak (2026-09-18 14:20)

Pod `nordic-27b-serve` (RunPod, one A100 SXM 80 GB, $1.59/h, US-MD-1, created through the RunPod MCP; EU-NL-1 had no A100 or H100 stock, so the weights volume there stays unused and the 27B was downloaded again, 52 GB in three minutes). vLLM 0.29 (torch 2.13, CUDA 13.0), `--max-model-len 16384 --gpu-memory-utilization 0.92 --max-num-seqs 64`, thinking off server-side. Through the RunPod proxy a schema-constrained round trip from the laptop takes about 1.0 s.

The clause-and few-shot bench on the pod's own vLLM: **0.8179** (accuracy 0.990, mean tIoU 0.703), against 0.8125 for the same run on the cluster's H100; paired +0.0053 ± 0.0026 with 4 spans better and none worse, i.e. bf16 kernel differences between the cards, not a change of behaviour. 3.8 s mean and 7.6 s worst per conversation of LLM time on the A100.

Live soak: the laptop endpoint (api.py under the supervisor, real turbo ASR on the laptop GPU, LLM calls to the pod through the proxy, `UNIT_SPLIT=clause-and`, `units-fewshot`, fallback `qwen3:4b`) took the 39 training conversations from `local_evaluator.py`: **score 0.815**, accuracy 0.992 (387 of 390), mean tIoU 0.697, 8.6 s mean and 16.3 s worst round trip, 0 timeouts, 0 fallbacks, 0 guesses (391 primary answers including the warm-up). The served path therefore reproduces the bench; the difference between 0.815 and 0.818 is live ASR against cached transcripts and the A100's numerics. Leakage check on the shipped pool: 5460 example lines over 390 questions, none from the tested conversation; no validation stem in the pool.
...
The pod's hostname is stable for the life of the pod and needs no laptop, tunnel or home network (committee report 02, finding 10; no tunnel exists today), so it is the URL to submit. The laptop endpoint stays up as the rehearsed alternative (its LLM calls go to the same pod).
