...
**Self-consistency (n=8 samples at T=0.7, served prompt, pod run `sc_bench.py`).** The run reached 250 of 390 questions (25 conversations, about 20 s per conversation on the A100) before the session's shell was torn down; the checkpoint `sc_units-fewshot_clause-and_n8_t0.7.partial.json` holds the 2,000 samples. On those 250 questions: greedy 0.8158 (A100) / 0.8095 (H100); MBR over 8 samples 0.8106; samples plus greedy weighted 1/2/4 0.8121 / 0.8101 / 0.8137; modal span 0.8112; oracle over samples 0.854. Self-consistency lands between the two greedy runs, i.e. on the noise floor. Sample agreement is calibrated (at n=160: unanimous spans mean tIoU 0.80, 50 % agreement 0.55) but of the positives where some sample beats greedy by 0.1, MBR picks that sample once. Shortest sampled span 0.7855, longest 0.8146: when samples disagree the longer candidate is right more often, a hint of the model's under-extension worth at most +0.003 and inside noise, not a recommendation.
