...
class ClassSpec:
    ...
    box_scale_margin: float = .06          # a scaled pose replaces the sprite-size pose only when it correlates this much better
    ...
    def colour_proposals(self, lab, s, zoom):
        ...
        for label in range(1, count):
            ...
            if area < cfg.get('min_area', 6) * s * s:
                continue
            # bbox crop: same pixels and point order as the full-image mask, without a full-image compare per component
            component = labels[y:y + h, x:x + w] == label
            pts = (np.column_stack(np.where(component))[:, ::-1] + (x, y)).astype(np.float32)
            ...
            rows.append(dict(cx=float(centroids[label][0]), cy=float(centroids[label][1]), heading=float(rotation), bar_angle=float(heading), proposer_score=float(probability[y:y + h, x:x + w][component].mean()),
    ...
    def detect(self, image, pixels_per_source_pixel=1., zoom=None, explain=False, proposals=None, scene_gpu=None):
        ...
        for pi, prop in enumerate(proposals):
            ...
            for extra in spec.box_scales:
                ...
                if hit2 is None or hit2[0] <= correlation + spec.box_scale_margin:
                    continue
                h2, w2 = tmask2.shape
