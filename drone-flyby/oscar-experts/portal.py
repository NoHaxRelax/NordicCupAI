...
def main():
    ...
    # Hard guard (Oscar, 19 Sep 2026): this client must never be able to reach the final evaluation.
    forbidden = ('evaluat', 'submit', 'final', 'test')
    if any(word in path.lower() for word in forbidden) or not (path == '/status' or path == '/verify' or path.startswith('/validate/')):
        raise SystemExit('Refusing non-validation route: ' + path)
