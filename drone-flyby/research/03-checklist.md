...
| A4 | Spend the latency headroom: 1536 input, flip averaging, two-checkpoint ensemble (`elias/ensemble.py`) | +0.02 to +0.05 at detector level | 2 | DOING 17:00: 1536 and 1024 input in `harness_campaign.log`, flip and scale augmentation (`ELIAS_AUGMENT=1`) in `harness_campaign2.log` | 110 ms of a 300 ms budget used on an RTX 5090; laptop 85 ms at 1280 |
...
| B2 | Confidence floor and thresholds (`DRONE_CONF=0.05 BIRTH 0.25 UPDATE 0.15`) re-tuned for the new model | +0.10 was the gain of lowering them the first time | 1 | DOING 17:00 on the laptop harness (`elias/out/logs/harness_campaign.log`: conf 0.03/0.10, birth 0.20/0.35, update 0.10/0.25 on F3 against 0.600) | harness = team labels, offline clock; a gain over 0.01 earns one concealed third on the portal |
| B3 | Emit forecast boxes for retired tracks at a low confidence instead of dropping them | small | 1 | IMPLEMENTED 17:05 (`DRONE_RETIRED_TICKS`, `DRONE_RETIRED_SCALE` in `tracking/revisit.py`), harness runs queued (`harness_campaign3.log`: 6 and 12 ticks at 0.3, 12 at 0.6, plus retirement after 5 misses) | low-confidence extras are nearly free |
| B5 | Context prior at runtime: boxes whose surroundings are water or forest keep their place at confidence x `ELIAS_CONTEXT` (never deleted) | precision on the classes with forest false positives | 0.5 | IMPLEMENTED 15:15 in `elias/ensemble.py`; harness runs at 0.5 and 0.3 queued (`harness_campaign2.log`); the portal one-class run was cut when the pods closed | same study |
