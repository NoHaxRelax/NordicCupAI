...
timeout 300 $POD "cd /root/work/drone-flyby && IMGSZ=${IMGSZ:-1280} ANSWER_CLASSES='${ANSWER_CLASSES:-}' CLASS_EXTENT='${CLASS_EXTENT:-}' HEDGE='${HEDGE:-0}' BOX_SCALE='${BOX_SCALE:-}' CLUSTER_BIRTHS='${CLUSTER_BIRTHS:-0}' L1_WAYPOINTS='${L1_WAYPOINTS:-4}' AUTO_BAND='${AUTO_BAND:-1}' DETECTOR='${DETECTOR:-ultralytics}' ELIAS_WEIGHTS='${ELIAS_WEIGHTS:-}' ELIAS_ROUTE='${ELIAS_ROUTE:-}' ELIAS_CONTEXT='${ELIAS_CONTEXT:-1.0}' ELIAS_CONF='${ELIAS_CONF:-0.05}' CUE_EVERY='${CUE_EVERY:-0}' CUE_PX='${CUE_PX:-40}' CUE_CONF='${CUE_CONF:-0.4}' CUE_COOLDOWN='${CUE_COOLDOWN:-12}' CUE_KIND='${CUE_KIND:-all}' CUE_CLASSES='${CUE_CLASSES:-}' DIRECT_URL='${DIRECT_URL:-}' bash elias/pod_serve.sh $W '$WIN' > /root/logs/serve_start.log 2>&1; cat /root/logs/serve.url"
