...
#            terrain 4.6 against 14.4 per frame; with the pair hedge harness 0.709, scene 0.638. NEEDS the morning's portal confirmation before it replaces routed.
...
WEIGHTS='/root/out/F3_both_m1280.last.pt,/root/out/F5_fixed_m1280.last.pt'; HEDGE_BOX='{}'; HEDGE_F=0; HEDGE_G=''
if [ "$MODE" = robust ] || [ "$MODE" = robust_hn ]; then
  ...
  # the look-alike pairs answered under each other's name at 0.3 x confidence (harness 0.696 to 0.709, exact-label scene
  # 0.615 to 0.638, no class loses; the default sibling groups cost small_launcher 0.014 and stay off)
  HEDGE_F=0.3; HEDGE_G='[["large_launcher","mine_roller"],["large_tower","medium_launcher"]]'
...
timeout 300 $POD "cd /root/work/drone-flyby && IMGSZ=1280 ANSWER_CLASSES='' CLASS_EXTENT='$CE' HEDGE=$HEDGE_F HEDGE_GROUPS='$HEDGE_G' BOX_SCALE='$BOX' CLUSTER_BIRTHS=1 L1_WAYPOINTS=4 AUTO_BAND=1 DETECTOR='$DET' ELIAS_WEIGHTS='$WEIGHTS' ELIAS_ROUTE='$ROUTE' BOX_HEDGE='$HEDGE_BOX' ELIAS_CONTEXT=1.0 ELIAS_CONF=0.05 DIRECT_URL='http://$H:$PUB' bash elias/pod_serve.sh /root/out/F3_both_m1280.last.pt '' > /root/logs/serve_start.log 2>&1; cat /root/logs/serve.url"
...
timeout 30 $POD 'grep -E "DRONE_ANSWER|DRONE_DETECTOR|DRONE_CLUSTER|DRONE_BOX|DRONE_HEDGE|DRONE_CLASS_EXTENT|ELIAS_WEIGHTS|ELIAS_ROUTE" /root/logs/serve.env | cut -c1-200'
