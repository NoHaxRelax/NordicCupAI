#!/bin/bash
# elias/pod_start_final.sh HOST SSH_PORT PUBLIC_PORT [routed|deploy]: start the endpoint FOR THE EVALUATION ATTEMPT on a
...
timeout 300 $POD "cd /root/work/drone-flyby && IMGSZ=1280 ANSWER_CLASSES='' CLASS_EXTENT='$CE' HEDGE=0 BOX_SCALE='$BOX' CLUSTER_BIRTHS=1 L1_WAYPOINTS=4 AUTO_BAND=1 DETECTOR='$DET' ELIAS_WEIGHTS='/root/out/F3_both_m1280.last.pt,/root/out/F5_fixed_m1280.last.pt' ELIAS_ROUTE='$ROUTE' ELIAS_CONTEXT=1.0 ELIAS_CONF=0.05 DIRECT_URL='http://$H:$PUB' bash elias/pod_serve.sh /root/out/F3_both_m1280.last.pt '' > /root/logs/serve_start.log 2>&1; cat /root/logs/serve.url"
