...
def observed_step(previous, previous_view, current, current_view, model, tick, fallback):
    ...
    """
    import cv2
    a, b = np.array(previous_view.region), np.array(current_view.region)
    region = np.r_[np.maximum(a[:2], b[:2]), np.minimum(a[2:], b[2:])]
    scale = np.maximum(previous_view.scale, current_view.scale)
    ...
    yy, xx = np.indices(size[::-1])
    source = np.stack([xx, yy], axis=-1)*scale+region[:2]+scale/2-.5
    patches = []
    for image, view in ((previous, previous_view), (current, current_view)):
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) if image.ndim == 3 else image
        local = (source+.5-view.region[:2])/view.scale-.5
        patches.append(cv2.remap(gray, local[:, :, 0].astype(np.float32), local[:, :, 1].astype(np.float32), cv2.INTER_LINEAR))
    ...
    def process(self, request, detections, *, image=None, tick=None, detector_ran=True, focus_box=None):
        ...
        if self.tracker is None:
            ...
            for d in sorted(detections, key=lambda item: -item.confidence):
                if d.confidence >= self.config.birth_confidence and counts.get(d.label, 0) < 100 and len(rows) < 500:
                    rows.append({'object_id': d.label, 'bbox': (view.box_to_source(d.box)/np.tile(view.source_size, 2)).tolist(), 'confidence': d.confidence})
