...
class DinoClassifier(nn.Module):
 def __init__(self,revision):
  super().__init__()
  self.backbone=torch.hub.load('facebookresearch/dinov2:'+revision,'dinov2_vits14',pretrained=True,trust_repo=True)
  self.head=nn.Linear(768,17)
 def forward(self,x):
  # Round up to patch multiple; never discard the edge of a reviewed square.
  side=((x.shape[-1]+13)//14)*14
  if x.shape[-1]!=side:x=F.interpolate(x,size=(side,side),mode='bilinear',align_corners=False)
  if not any(p.requires_grad for p in self.backbone.parameters()):
   self.backbone.eval()
   with torch.no_grad():f=self.backbone.forward_features(x)
  else:f=self.backbone.forward_features(x)
  return self.head(torch.cat([f['x_norm_clstoken'],f['x_norm_patchtokens'].mean(1)],dim=1))
...
def run(a):
 ...
 if a.arch=='dino-small':model=DinoClassifier(a.dino_revision)
 elif a.arch=='small':model=SmallCNN()
 else:
 ...
 model.to(device);head=model.head if a.arch in ('small','dino-small') else model.fc
 ...
 for epoch in range(a.epochs):
  started=time.time();model.train();frozen=a.freeze_backbone or (a.pretrained and epoch<2)
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--data',required=True);p.add_argument('--output');p.add_argument('--arch',choices=['small','resnet18','resnet34','dino-small'],default='resnet18');p.add_argument('--pretrained',action='store_true');p.add_argument('--freeze-backbone',action='store_true');p.add_argument('--dino-revision',default='7764ea0f912e53c92e82eb78a2a1631e92725fc8');p.add_argument('--cpu',action='store_true');p.add_argument('--seed',type=int,default=1731);p.add_argument('--batch',type=int,default=32);p.add_argument('--epochs',type=int,default=24);p.add_argument('--steps',type=int,default=50);p.add_argument('--lr',type=float,default=.001);p.add_argument('--patience',type=int,default=6);run(p.parse_args())
