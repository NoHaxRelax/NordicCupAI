...
class RevisitConfig:
    ...
    # Confidence memory: a track that has been seen with strong confidence many times is not
    # pulled straight down by one weak sighting. A refresh at or above strong_confidence adds one
    # strong sighting (capped at confidence_memory); a weaker refresh removes one. A downward
    # refresh moves the stored confidence only 1/(1+strong sightings) of the way to the new value,
    # so it takes about as many weak sightings as there were strong ones to forget an object.
    # Upward refreshes still apply immediately. 0 disables the memory (old immediate overwrite).
    confidence_memory: int = 0
    strong_confidence: float = .5
    # How a downward refresh is damped once confidence_memory > 0:
    #  'count': old-(old-new)/(1+strong sightings)          (strength = confidence_memory)
    #  'ema':   old-(old-new)*confidence_alpha, alpha in (0,1] (strength = 1/alpha; strong sightings unused)
    #  'peak':  max(new, old*confidence_decay)               (strength = decay; strong sightings unused)
    confidence_mode: str = 'count'
    confidence_alpha: float = .5
    confidence_decay: float = .9
    ...
    def __post_init__(self):
        ...
        if not 0 <= float(finite(self.strong_confidence, 'strong_confidence')) <= 1:
            raise ValueError('strong_confidence must lie in [0,1]')
        if int(self.confidence_memory) != self.confidence_memory or self.confidence_memory < 0:
            raise ValueError('confidence_memory must be a nonnegative integer')
        if self.confidence_mode not in ('count', 'ema', 'peak'):
            raise ValueError("confidence_mode must be 'count', 'ema' or 'peak'")
        if not 0 < float(finite(self.confidence_alpha, 'confidence_alpha')) <= 1:
            raise ValueError('confidence_alpha must lie in (0,1]')
        if not 0 <= float(finite(self.confidence_decay, 'confidence_decay')) <= 1:
...
class RevisitedTrack:
    ...
    strong_sightings: int = 0  # recent whole sightings at or above strong_confidence, capped at confidence_memory
...
class RevisitTracker:
    ...
    def _remembered_confidence(self, track, confidence):
        """The confidence to store after a whole sighting at `confidence`.

        An object the detector was sure about for a long time keeps most of that
        certainty through a single unsure sighting; repeated unsure sightings
        erode it one strong sighting at a time. Rises are never damped.
        """
        memory = int(self.config.confidence_memory)
        confidence = float(confidence)
        if memory <= 0:
            track.strong_sightings = 0
            return confidence
        weight = min(track.strong_sightings, memory)
        old = float(track.confidence)
        if confidence >= old:
            stored = confidence
        elif self.config.confidence_mode == 'ema':
            stored = old-(old-confidence)*self.config.confidence_alpha
        elif self.config.confidence_mode == 'peak':
            stored = max(confidence, old*self.config.confidence_decay)
        else:
            stored = old-(old-confidence)/(1+weight)
        if confidence >= self.config.strong_confidence:
            track.strong_sightings = min(memory, track.strong_sightings+1)
        else:
            track.strong_sightings = max(0, track.strong_sightings-1)
        return stored
    ...
    def _accept(self, track, box, tick, frame, confidence):
        ...
        track.confidence = self._remembered_confidence(track, confidence)
    ...
    def update(self, detections, view, tick, frame_index, *, detector_ran=True):
        ...
        self.pre_update_predictions = self.predictions(tick)
        candidates = []
        had_candidate = set()
        ...
        for index, (d, box, complete) in enumerate(kept):
            ...
            if index in had_candidate:
                candidate_ids = {identity for _, i, identity in candidates if i == index}
                # A second distinct object must not be lost just because it was
                # within the loose distance gate of an already matched track.
                # Genuine unresolved alternatives and overlapping duplicates
                # remain ineligible for birth.
                distinct = (candidate_ids <= matched_tracks and all(
                    overlap(box, self._box(self.tracks[k], tick)) < self.config.association_iou
                    for k in candidate_ids))
                if not distinct:
                    self.events.append({'event': 'ambiguous_detection', 'label': d.label,
                                        'box': box.tolist()})
                    continue
                self.events.append({'event': 'distinct_neighbor', 'label': d.label,
                                    'box': box.tolist()})
            ...
            if self.config.confidence_memory and d.confidence >= self.config.strong_confidence:
                self.tracks[identity].strong_sightings = 1
            matched_tracks.add(identity)
