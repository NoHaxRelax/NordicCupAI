...
Configuration is by environment variables (defaults in brackets):
  ...
  DRONE_CONFIDENCE_MEMORY, DRONE_STRONG_CONFIDENCE  strong sightings remembered before a weak one
                            can pull a track's confidence down (0 = overwrite at once), strong threshold [0, 0.5]
  DRONE_CONFIDENCE_MODE     count | ema | peak damping of downward refreshes; DRONE_CONFIDENCE_ALPHA,
                            DRONE_CONFIDENCE_DECAY are the ema/peak strengths             [count, 0.5, 0.9]
...
if os.environ.get('DRONE_FAST_CALIBRATION', '0') == '1':
    from fast_motion import install
    install()
...
CONFIG = RevisitConfig(
    ...
    # An object seen with strong confidence for a long time is not pulled straight down by one weak
    # sighting: a downward refresh moves 1/(1+strong sightings) of the way, strong sightings capped here.
    confidence_memory=int(os.environ.get('DRONE_CONFIDENCE_MEMORY', '0')),
    strong_confidence=float(os.environ.get('DRONE_STRONG_CONFIDENCE', '0.5')),
    confidence_mode=os.environ.get('DRONE_CONFIDENCE_MODE', 'count'),
    confidence_alpha=float(os.environ.get('DRONE_CONFIDENCE_ALPHA', '0.5')),
    confidence_decay=float(os.environ.get('DRONE_CONFIDENCE_DECAY', '0.9')),
)
...
def predict(request: DroneFlybyPredictRequestDto) -> DroneFlybyPredictResponseDto:
    ...
    with session.lock:
        # A timed-out request can arrive after a newer frame on another connection.
        # Preserve the current tracker and avoid spending GPU time on stale pixels.
        if session.workflow.last_frame is not None and req['frame_index'] <= session.workflow.last_frame:
            if req['request_id'] == session.workflow.last_request_id:
                return DroneFlybyPredictResponseDto.model_validate(session.workflow.last_response)
        ...
        session.record({'frame': req['frame'], 'frame_index': req['frame_index'], 'level': req['view']['resolution_level'],
                        ...
                        'track_details': diagnostics.get('tracks', []),
                        'pre_update_predictions': diagnostics.get('pre_update_predictions', []),
                        'focus_box': diagnostics.get('focus_box'),
