...
void apply_activation_overrides(orchard::Params& p,const J& config,int mode){
 ...
 std::map<std::string,bool*> bool_fields={{"heir_select",&p.heir_select},{"feed_breed",&p.feed_breed}};
...
int main(int argc,char**argv){
 ...
 while(!e->agents.empty() && e->time+1e-6<horizon){
  ...
  if(((mode>=35&&mode<=38)||(mode>=140&&mode<=248))&&p.resource_synchronized)safety.apply(*e,p,acts,mode);
