...
int main(int argc,char**argv){
 ...
 while(!e->agents.empty() && e->time+1e-6<horizon){
  ...
  if(mode>=7){p.model_agents.clear();p.model_predators.clear();p.model_obstacles.clear();for(auto&a:e->agents)p.model_agents.push_back({a.id,{a.x,a.y},a.direction});for(auto&a:e->predators)p.model_predators.push_back({a.key,{a.x,a.y},a.direction,a.resting});for(auto&o:e->obstacles)p.model_obstacles.push_back({o.x,o.y,o.w,o.h});}
  if(!activated&&p.resource_synchronized){activated=true;if(mode==12||mode==14){p.P.breed_reserve+=60.;p.P.breed_reserve_late+=60.;}if(mode==24||mode==25){p.P.tree_reach*=2.;p.P.fruit_reach*=2.;}}
  ...
  for(auto&a:acts)e->agent_step({a.aid,a.dist,true,a.direction,a.turn,a.spawn});
