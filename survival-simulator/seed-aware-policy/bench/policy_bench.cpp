...
int main(int argc,char**argv){
 ...
 while(!e->agents.empty() && e->time+1e-6<horizon){
  ...
  if(((mode>=35&&mode<=38)||(mode>=140&&mode<=188))&&p.resource_synchronized)safety.apply(*e,p,acts,mode);
