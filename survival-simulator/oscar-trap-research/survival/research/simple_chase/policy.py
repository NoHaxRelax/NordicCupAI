    ...
    def act(self, observations, sim_time):
        ...
        for state in observations:
            ...
            if pose is None:
                ...
                continue
            if nearest:
                self.last_predator_point = pose.point(nearest)
            ...
            if self.phase=='route' and math.dist(pose.p,far)<6:
                self.phase='align'; self.route=[]
            if self.phase=='align' and nearest and distance<65:
                axial=sum(a*b for a,b in zip(sub(pred,pose.p), self.site['inward']))
                if axial<0: self.phase='final'
            if self.phase=='final':
                # Previously demonstrated axial lead followed by guide sacrifice.
                actions.append(self._move(aid,state,pose,tuple(self.site['hold']),'final_axial_lead_and_hold',turn=0.,sprint=True)); continue
            ...
            if distance>75:
                desired=add(pred,mul(unit(sub(pose.p,pred)),55.))
            ...
            for sprint in (False,True):
                for k in range(24):
                    ...
                    if sep<48:continue
                    score=math.dist(pose.p,desired)-math.dist(endpoint,desired)-.7*abs(sep-65)-(.5 if sprint else 0)
                    candidates.append((score,endpoint,sprint))
            # Holding is valid when safely separated; avoids running off during rest.
            if distance>=48:candidates.append((-.7*abs(distance-65),pose.p,False))
            if candidates:
                _,target,sprint=max(candidates,key=lambda row:row[0])
                actions.append(self._move(aid,state,pose,target,'continuous_gaze_distance_control',turn=turn,sprint=sprint))
            else:
