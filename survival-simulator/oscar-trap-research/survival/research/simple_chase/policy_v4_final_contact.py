    ...
    def act(self, observations, sim_time):
        ...
        for state in observations:
            ...
            if nearest:
                ...
                self.pred_motion = math.dist(observed,self.last_predator_point) if self.last_predator_point is not None else 0.
            ...
            if self.phase=='approach' and math.dist(pose.p,far)<8 and nearest and distance<80:
                ...
                if axial<0 and cross<20 and self.pred_motion>1.: self.phase='final'
            if self.phase=='final':
                ...
                if nearest is None or self.stationary_ticks>=3:
                    # Rest or contact loss invalidates the final lead. Stay near
                    # the observed follower and obtain movement evidence again.
                    self.phase='approach'; self.route=[]
                    far=tuple(self.site['far'])
                else:
                    actions.append(self._move(aid,state,pose,tuple(self.site['hold']),'final_observed_axial_lead',turn=turn,sprint=True)); continue
            ...
            for sprint in (False,True):
                for k in range(24):
                    ...
                    sep=math.dist(endpoint,pred)
                    if sep<50:continue
                    score=math.dist(pose.p,desired)-math.dist(endpoint,desired)-.7*abs(sep-58)-(.5 if sprint else 0)
                    candidates.append((score,endpoint,sprint))
            # Holding is valid when safely separated; avoids running off during rest.
            if distance>=50:candidates.append((-.7*abs(distance-58),pose.p,False))
            if candidates:
                _,target,sprint=max(candidates,key=lambda row:row[0])
                actions.append(self._move(aid,state,pose,target,'continuous_gaze_replanned_route',turn=turn,sprint=sprint))
            else:
                actions.append(self._evade(aid,state,pose,pred,'emergency_escape',turn=turn,desired=desired))
        return actions
