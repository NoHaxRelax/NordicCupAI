"""Fixed fresh map/fixture pairs for frozen v15; four bounded fresh workers."""
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor,as_completed
import subprocess,sys,json
ROOT=Path(__file__).resolve().parents[2]
CASES=[(10000+i,9600+i) for i in range(32,48)]
OUT=ROOT/'results/simple_chase/heldout-v15-execution.json'
rows=[]
def run(case):
 ...
 command=['systemd-run','--user','--scope','-p','MemoryMax=3G','-p','MemoryHigh=2G','-p','MemorySwapMax=0','--',sys.executable,
          str(ROOT/'research/simple_chase/run_streaming_v2.py'),'--policy','simple_chase.policy_v15_direct_escape:SimpleChase',
          '--seconds','180','--map-seed',str(case[0]),'--fixture-seed',str(case[1]),'--station-bait','--native-width','320']
 with (ROOT/f'results/simple_chase/heldout-v15-m{case[0]}.log').open('w') as log:
  result=subprocess.run(command,cwd=ROOT,stdout=log,stderr=subprocess.STDOUT)
 return {'map_seed':case[0],'fixture_seed':case[1],'exit_code':result.returncode}
OUT.write_text(json.dumps({'policy':'v15 frozen','cases':CASES,'executed':rows},indent=2)+'\n')
