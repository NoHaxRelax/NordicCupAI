        ...
        for state in observations:
            ...
            for cap in (3.,6.,10.,15.,20.):
                ...
                for k in range(24):
                    ...
                    upper=58. if nearest and abs(nearest.get('rel_dir',0.))>.5 else 65.
                    if sep>upper and distance<=80:continue
                    score=.7*(math.dist(pose.p,desired)-math.dist(endpoint,desired))-.8*abs(sep-55)-(.2 if sprint else 0)
                    candidates.append((score,endpoint,sprint))
            # Holding is valid when safely separated; avoids running off during rest.
            if 50<=distance<=65:candidates.append((-.8*abs(distance-55),pose.p,False))
            if candidates:
                _,target,sprint=max(candidates,key=lambda row:row[0])
                actions.append(self._move(aid,state,pose,target,'continuous_gaze_bounded_hearing_spacing',turn=turn,sprint=sprint))
            else:
