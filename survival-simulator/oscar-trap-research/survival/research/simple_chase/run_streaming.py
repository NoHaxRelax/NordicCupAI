...
from streaming_recorder import ReplayRecorder
...
def _refresh_visualization_manifest():
    return {"status": "deferred", "reason": "bounded-memory runs defer full-replay viewer processing"}


def _harness_hashes():
    paths = [Path(__file__), HERE / "sampling.py", HERE / "static_map.py", ROOT / "debugger/streaming_recorder.py"]
...
def run(*, policy_spec="smoke_policy:SmokePolicy", map_seed=5101,
    ...
    recorder = ReplayRecorder(
        ...
        notes=("SETUP OVERRIDE: guide starts 55 units from predator and faces it; predator starts awake at full energy facing guide. Native predator energy/rest thereafter. No child. Bait is predeployed. Runtime policy receives no hidden creature data. Original harness description below is superseded for initial placement. " + __doc__ + " Replay interval is exactly one native 0.1-second tick. "
