...
class SimpleChase(RealMapGuidePolicy):
        ...
        for state in observations:
            ...
            if pose is None:
                ...
                if nearest:
                    ...
                    translation=max(-state['speed']*modifier,min(state['sprint_speed']*modifier,70.-nearest['distance']))
                    direction=wrap(nearest['angle']+(math.pi if translation>=0 else 0.))
                    actions.append(dict(agent_id=aid,move_distance=abs(translation)/modifier,
            ...
            radius=getattr(self,'route_radius',20.)
            if self.route and not self._clear(pose.p,self.route[0],radius):self.route=[]
            if not self.route:
                for radius in (20.,15.,11.):
                    self.route=self._path(pose.p,far,radius)
                    if self.route:
                        self.route_radius=radius;break
            self._advance_route(pose,self.route,radius)
            ...
            for cap in (3.,6.,10.,15.,20.):
                ...
                for k in range(24):
                    ...
                    score -= 20. if not self._free(endpoint,18.) else 0.
    ...
    def _evade(self,aid,state,pose,pred,rule,turn=0.,desired=None):
        # When normal spacing has no feasible candidate, first maximize distance
        # from the last observed predator, rather than sacrificing escape to route
        # progress or an unreliable point-velocity prediction.
        ...
        for k in range(48):
            ...
            if self._clear(pose.p,endpoint,5.01):
                clearance_bonus=20. if self._free(endpoint,18.) else 0.
                candidates.append((math.dist(endpoint,observed)+clearance_bonus,endpoint))
        target=max(candidates,key=lambda row:row[0])[1] if candidates else pose.p
        return self._move(aid,state,pose,target,'maximize_observed_escape_distance',turn=turn,sprint=True)
