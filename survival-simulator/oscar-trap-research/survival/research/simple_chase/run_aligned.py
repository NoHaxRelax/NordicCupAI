...
def _harness_hashes():
    return {name: hashlib.sha256((HERE / name).read_bytes()).hexdigest()
            for name in (Path(__file__).name, "sampling.py", "static_map.py")}
...
def _spawn_fixture(env, fixture: dict, *, site: dict, station_bait: bool):
    ...
    if station_bait:
        if env._in_obstacle(tuple(site["goal"]), radius=5., obstacles=env.obstacles):
            raise ValueError("bait goal intersects native agent radius")
        rows["agents"][0]["arranged_at_site"] = True
    # Disclosed setup only: place guide 55 units from predator, facing it.
    from sampling import _free_for_native_spawn
    pred = rows["predators"][0]
    for k in range(72):
        angle = pred["heading"] + k * math.tau / 72
        point = (pred["x"] + 55 * math.cos(angle), pred["y"] + 55 * math.sin(angle))
        if _free_for_native_spawn(env, point, 5, "agent") and math.dist(point, site["goal"]) > 100:
            rows["agents"][1].update(x=point[0], y=point[1], heading=(angle + math.pi) % math.tau, arranged_next_to_predator=True)
            pred["heading"] = angle
            break
    else:
    ...
    for row in rows["agents"]:
        ...
        if row.get("arranged_at_site"):
            agent.x, agent.y = site["goal"]
            row.update(x=agent.x, y=agent.y, setup_repositioned=True)
...
def run(*, policy_spec="smoke_policy:SmokePolicy", map_seed=5101,
    ...
    fixture = sample_fixture(env, fixture_seed=fixture_seed, agents=2,
                             predators=predators)
    fixture["predators"][0].update(
        x=site["mouth"][0]-180*site["inward"][0],
        y=site["mouth"][1]-180*site["inward"][1],
        heading=math.atan2(site["inward"][1],site["inward"][0]),
        arranged_aligned_final_approach=True)
    policy.phase = "approach"
    ...
    recorder = ReplayRecorder(
        ...
        scenario="arranged adjacent guide 55 units from awake predator; predeployed bait; native map",
        notes=("SETUP OVERRIDE: ALIGNED FINAL APPROACH CONTROL, predator180 outside mouth, guide125 outside, not random anywhere. Guide starts 55 units from predator and faces it; predator starts awake at full energy facing guide. Native predator energy/rest thereafter. No child. Bait is predeployed. Runtime policy receives no hidden creature data. Original harness description below is superseded for initial placement. " + __doc__ + " Replay interval is exactly one native 0.1-second tick. "
    ...
    result = json_value({
        ...
        "predator_state_native": False,
        "predator_dynamics_native_after_setup": True,
        "arranged_adjacent_awake_start": True,
        "arranged_aligned_final_approach": True,
        "starts": starts,
