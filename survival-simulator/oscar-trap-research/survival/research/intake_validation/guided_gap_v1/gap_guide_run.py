...
from observed_gap_guide_policy import ObservedGapGuidePolicy
...
OUT=ROOT/"results"/"intake_validation"/"guided_gap_v1"
POLICY_HASH=hashlib.sha256((HERE/"observed_gap_guide_policy.py").read_bytes()+
                           (HERE/"observed_gap_base_snapshot.py").read_bytes()).hexdigest()
...
def run(predators=4,interval=90.,seconds=400.,seed=4301,gap=15.,length=90.,
    ...
    cx,mouth_y=800.,500.;ow=60.
    left=Obstacle(cx-gap/2-ow,mouth_y,ow,length);right=Obstacle(cx+gap/2,mouth_y,ow,length)
    ...
    bait=add_agent(env,cx,mouth_y-15,energy=150);bait.direction=math.pi/2
    env._next_agent_id=1;env._update_spatial_grid()
    policy=ObservedGapGuidePolicy();tag=f"observed-gap-guides-g{gap:g}-n{predators}-i{interval:g}-lat{lateral:g}-s{seed}-{uuid4().hex[:8]}"
    replay=OUT/"replays"/f"{tag}.json.gz"
    rec=ReplayRecorder(env,title=tag,policy="observation-only-off-axis-gap-guides-v1",seed=seed,every=every,
    ...
    target_switch_ticks={};deaths=[];tail=[];physical_tail=[];trace=[];max_mapped=0
    for tick in range(round(seconds*10)):
        ...
        if idx<predators and env.time+.001>=due:
            ...
            gx=cx+side*lateral;gy=mouth_y-approach
            vx,vy=gx-cx,gy-mouth_y;scale=follower_gap/math.hypot(vx,vy)
            ...
            guide.direction=math.atan2(mouth_y-gy,cx-gx)
        ...
        safe={a.agent_id for a in env.agents if mouth_y+5<=a.y<=mouth_y+length-5 and abs(a.x-cx)<=gap/2-5+.01}
        joint=physical_count=0
        for item in arrivals:
            p=item["predator"];physical=(math.dist((p.x,p.y),(cx,mouth_y))<=75 and p.y-mouth_y<=10)
            ...
            if p in acquired and not p.resting and chosen not in safe:target_switch_ticks[p]+=1
    ...
    for item in arrivals:
        ...
        rows.append(dict(**item,guide_id=g.agent_id,guide_alive=g.agent_id in env.agents_dict,acquired=acquired.get(p),physical_acquired=physical_acquired.get(p),first_joint_loss=losses.get(p),first_physical_loss=physical_losses.get(p),active_target_switch_ticks=target_switch_ticks[p]))
    n=min(300,len(tail));result=dict(policy_hash=POLICY_HASH,seed=seed,predators=predators,interval=interval,seconds=round(env.time,1),requested_seconds=seconds,gap=gap,length=length,lateral=lateral,approach=approach,follower_gap=follower_gap,arrivals=len(arrivals),mapped=max_mapped,agents_alive=len(env.agents),guides_alive=sum(r["guide_alive"] for r in rows),deaths=deaths,acquired=len(acquired),physical_acquired=len(physical_acquired),joint_losses=len(losses),physical_losses=len(physical_losses),final_joint=tail[-1],final_physical=physical_tail[-1],tail_joint_min=min(tail[-n:]),tail_physical_min=min(physical_tail[-n:]),joint_success=len(arrivals)==predators and len(acquired)==predators and not losses and env.time>=seconds-.01 and min(tail[-n:])==predators,physical_success=len(arrivals)==predators and len(physical_acquired)==predators and not physical_losses and env.time>=seconds-.01 and min(physical_tail[-n:])==predators,deliveries=rows,trace=trace,replay=str(replay.relative_to(ROOT)))
    result=json_value(result)
    OUT.mkdir(parents=True,exist_ok=True);(OUT/f"{tag}.json").write_text(json.dumps(result,indent=2)+"\n");return result

if __name__=="__main__":
    p=argparse.ArgumentParser(description=__doc__)
    for name,default in [("predators",4),("interval",90.),("seconds",400.),("seed",4301),("gap",15.),("length",90.),("lateral",60.),("approach",150.),("follower-gap",65.),("heading-jitter",.2),("start-delay",6.),("every",10)]:
