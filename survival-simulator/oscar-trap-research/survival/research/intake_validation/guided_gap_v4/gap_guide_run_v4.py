...
OUT=ROOT/"results"/"intake_validation"/"guided_gap_v4"
...
def run(predators=4,interval=90.,seconds=400.,seed=4301,gap=15.,length=90.,
    ...
    policy=ObservedGapGuidePolicy();tag=f"observed-gap-guides-v4-aligned-hold-g{gap:g}-l{length:g}x{right_length:g}-o{face_offset:g}-h{int(horizontal)}-n{predators}-i{interval:g}-lat{lateral:g}-s{seed}-{uuid4().hex[:8]}"
