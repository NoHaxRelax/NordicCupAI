...
from observed_shallow_gap_policy import ObservedGapGuidePolicy
...
OUT=ROOT/"results"/"intake_validation"/"shallow_gap"
POLICY_HASH=hashlib.sha256((HERE/"observed_shallow_gap_policy.py").read_bytes()+
...
def run(predators=4,interval=90.,seconds=400.,seed=4301,gap=15.,length=90.,
    ...
    policy=ObservedGapGuidePolicy();tag=f"shallow-gap-d15-hold42.5-g{gap:g}-l{length:g}x{right_length:g}-o{face_offset:g}-h{int(horizontal)}-n{predators}-i{interval:g}-lat{lateral:g}-s{seed}-{uuid4().hex[:8]}"
    replay=OUT/"replays"/f"{tag}.json.gz"
    rec=ReplayRecorder(env,title=tag,policy="observation-only-shallow-gap-bait-d15-hold42.5-v1",seed=seed,every=every,
    ...
    deaths=[];tail=[];physical_tail=[];trace=[];max_mapped=0;safe=set();min_bait_predator_distance=float("inf")
    for tick in range(round(seconds*10)):
        ...
        if env.predators:
            min_bait_predator_distance=min(min_bait_predator_distance,
                min(math.dist((p.x,p.y),(bait.x,bait.y)) for p in env.predators))
