...
class RelayController:
    def __init__(self, bait_ids, variant='mpc', switch_energy=260, dwell=2., target_radius=53,
                 recovery=True, newborn_ready=180, pair_period=0., home_weight=.0003, assignment_weight=12, relay=False, forage=False, renew=False, birth_energy=225, birth_age=25, **unused):
        ...
        self.birth_energy=birth_energy;self.birth_age=birth_age;self.relay=relay;self.forage=forage;self.renew=renew;self.rest_ticks=0;self.handoff_phase=None;self.center=None;self.forage_targets={};self.known_ids=set(bait_ids);self.last_birth=-100.
            ...
            for aid,s in rows.items():
                ...
                for o in s['observations']:
                    if o['type']=='Agent' and o['id'] not in self.pose:
                        ...
                        if self.pending_parent is not None and o['id'] not in self.known_ids:
    ...
    def choose_orbit(self,ss,t):
        ...
        # Preserve the last moving orbit center while the predator is still.
        if self.pred_speed>1 or self.center is None:self.center=center.copy()
        else:center=self.center.copy()
        if self.relay and self.incoming is not None and self.rest_ticks>=2 and self.handoff_phase is None:
            self.handoff_phase='approach';self.events.append(dict(t=t,event='incoming_approach',agent=self.incoming))
        if self.handoff_phase=='approach' and self.incoming in self.pose and np.linalg.norm(self.pose[self.incoming][0]-center)<8:
            old=self.active;self.active=self.incoming;self.incoming=None;self.last_switch=t;self.handoff_phase=None
            self.events.append(dict(t=t,event='center_handoff',outgoing=old,incoming=self.active));self.retiring.add(old)
            self.retiring.discard(self.active)
        moves={}
        for s in ss:
            aid=s['agent_id'];pos,theta=self.pose[aid]
            ...
            if aid==self.active or (aid==self.incoming and self.handoff_phase=='approach'):
                ...
                staging=center+(pos-center)/max(np.linalg.norm(pos-center),1)*120
                if self.forage and s['energy']<430:
                    fruit=[fp for fp,seen_t in self.food.values() if np.linalg.norm(fp-center)>75 and np.linalg.norm(fp-p)>65]
                    if fruit:staging=min(fruit,key=lambda fp:np.linalg.norm(fp-pos))
    ...
    def __call__(self,states,t):
        ...
        if self.active not in rows:
            ready=[s for s in live if s['energy']>=max(self.newborn_ready,s['max_energy']/5+25)]
            self.active=max(ready,key=lambda s:s['energy'])['agent_id'] if ready else None
        if self.incoming not in rows:self.incoming=None
        if self.active is not None and len(live)>1 and (self.variant!='orbit' or self.relay):
            current=rows[self.active]
            candidates=[s for s in live if s['agent_id']!=self.active and s['energy']>=max(self.newborn_ready,s['max_energy']/5+25) and (s['energy']>current['energy']+30 or (self.renew and current['age']>60 and s['age']<current['age']-20))]
            urgent=current['energy']<self.switch_energy or (self.renew and current['age']>60)
            ...
            if candidates and (urgent or timed) and t-self.last_switch>self.dwell and self.incoming is None:
                self.incoming=max(candidates,key=lambda s:s['energy']-3*s['age'])['agent_id']
        ...
        if self.pred and t-self.last_seen<.5 and live:
            chosen=sorted(live,key=lambda s:(s['agent_id']!=self.active,s['agent_id']!=self.incoming,-s['energy']))
            if self.variant!='orbit':chosen=chosen[:2]
        ...
        for s in states:
            ...
            if aid in self.pose:
                pos,theta=self.pose[aid]
                if aid in moves:
                    v=moves[aid];turn=wrap(bearing(self.pred[0]-pos-v)-theta);why='active' if aid==self.active else 'incoming' if aid==self.incoming else 'reserve forage'
                    if aid!=self.active and aid!=self.incoming and np.linalg.norm(v)<.1:turn=.35
                    if self.renew and aid!=self.active and len(live)<3 and s['energy']>self.birth_energy and (s['age']>self.birth_age or len(live)<2) and len(self.food)>=2 and t-self.last_birth>15 and np.linalg.norm(pos-self.pred[0])>100:
                        spawn=True;self.pending_parent=aid;self.last_birth=t;self.events.append(dict(t=t,event='birth_requested',parent=aid))
                ...
                # A worker need not share the bait's coordinate frame to forage.
                pp=[o for o in s['observations'] if o['type']=='Predator'];ff=[o for o in s['observations'] if o['type']=='Fruit']
                distance=direction=0.
                if pp and min(o['distance'] for o in pp)<130:
                    q=min(pp,key=lambda o:o['distance']);distance=min(s['sprint_speed'],20 if s['energy']>=s['max_energy']/5 else s['speed']);direction=wrap(q['angle']+math.pi);turn=q['angle'];why='worker escape'
                elif ff:
                    q=min(ff,key=lambda o:o['distance']);distance=min(s['speed'],max(0,q['distance']-4));direction=turn=q['angle'];why='worker forage'
...
class MultiRelayController:
    """Two local controllers with disjoint initial pairs; no predator IDs in policy.

    This tests preassigned pair ownership, not global predator discovery or an
    optimal dynamic matching solver. Each pair associates a predator from its
    own members' observations and then follows the nearest track continuation.
    """
    def __init__(self, groups, variant='orbit', **params):
        self.controllers=[RelayController(group,variant=variant,**params) for group in groups]
        self.workers=RelayController([],variant='mpc')
        self.initial=set().union(*(set(group) for group in groups));self.known=set(self.initial)
        self.baits=set(self.initial);self.events=[];self.event_counts=[0]*len(groups)
        self.newborn_ready=params.get('newborn_ready',140);self.active=None;self.incoming=None

    def __call__(self,states,t):
        rows={s['agent_id']:s for s in states};taken=set().union(*(c.baits for c in self.controllers))
        for c in self.controllers:
            if c.pending_parent is None:continue
            parent=rows.get(c.pending_parent)
            if parent:
                babies=[o['id'] for o in parent['observations'] if o['type']=='Agent' and o['id'] not in self.known and o['distance']<=50]
                if babies:c.baits.add(babies[0]);taken.add(babies[0])
        self.known.update(rows)
        action_map={};decisions={}
        for index,c in enumerate(self.controllers):
            own=[s for s in states if s['agent_id'] in c.baits]
            actions,why=c(own,t)
            for action in actions:action_map[action['agent_id']]=action
            decisions.update({aid:f'pair {index}: {w}' for aid,w in why.items()})
            self.events.extend(dict(pair=index,**v) for v in c.events[self.event_counts[index]:]);self.event_counts[index]=len(c.events)
        self.baits=set().union(*(c.baits for c in self.controllers))
        actions,why=self.workers([s for s in states if s['agent_id'] not in self.baits],t)
        for action in actions:action_map[action['agent_id']]=action
        decisions.update(why)
        self.active=[c.active for c in self.controllers];self.incoming=[c.incoming for c in self.controllers]
        return list(action_map.values()),decisions
