...
class RelayController:
    def __init__(self, bait_ids, variant='mpc', switch_energy=260, dwell=2., target_radius=53,
                 recovery=True, newborn_ready=180, pair_period=0., home_weight=.0003, assignment_weight=12, relay=False, forage=False, renew=False, **unused):
        ...
        self.relay=relay;self.forage=forage;self.renew=renew;self.rest_ticks=0;self.handoff_phase=None;self.center=None;self.forage_targets={};self.known_ids=set(bait_ids);self.last_birth=-100.
        ...
        for s in states:
            ...
            if aid in self.pose:
                ...
                if aid in moves:
                    ...
                    if self.renew and aid!=self.active and len(live)<3 and s['energy']>260 and (s['age']>45 or len(live)<2) and t-self.last_birth>15 and np.linalg.norm(pos-self.pred[0])>100:
