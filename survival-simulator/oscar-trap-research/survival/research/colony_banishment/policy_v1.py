...
class LocalMap:
    ...
    """
    def __init__(self):
        self.poses={}; self.features={}; self.trees={}; self.homes={}
    ...
    def update(self, states, t, oracle=None):
        ...
        if oracle is not None:
            ...
            for s in states:
                ...
                if residual:
                    dx,dy=(statistics.median(v[i] for v in residual) for i in (0,1))
                    p.x+=dx;p.y+=dy
        ...
        for s in states:
            ...
            for o in s['observations']:
                if o['type']=='Tree':
                    ...
                    fs.extend([('Edge',project(p.xy(),p.theta+math.atan2(y,x),math.hypot(x,y))) for x,y in o['coords']])
...
class ColonyPolicy:
    def __init__(self, banish=False, *, variant='rest', min_fuel=300., population=4,
                 release_radius=450., guide_limit=18., guard=False):
        ...
        self.guide_limit=guide_limit;self.guard=guard
    ...
    def safe_heading(self,s,desired,distance,pred=None):
        ...
        def score(a):
            pts=[(r*math.cos(a),r*math.sin(a)) for r in (distance/2,distance,distance+12)]
    ...
    def __call__(self, states, t, *, oracle_poses=None):
        ...
        for s in ordered:
            ...
            if aid==self.guide:
                ...
                if self.phase=='lead' and remote and (self.variant=='distance' or self.still>=2):
                ...
                if self.phase=='lead':
                    ...
                    if pred:
                        ...
                        heading=away+max(-1.,min(1.,offset))
                        d=pred['distance']
                        request=max(walk,min(20.,speed+.5))
                        move=min(s['sprint_speed'],request) if d<78 else (walk if d<110 else 0.)
                        turn=pred['angle'];direction=wrap(heading-p.theta)
                        why='lead away from orchard'
                    else:
                        direction=wrap(outward-p.theta);move=walk if t-self.last_seen<.6 else 0.;turn=.22
                ...
                if danger:
                    ...
                    trees.sort(key=lambda q:(q[0],q[1]))
            ...
            a=ActionRequest(agent_id=aid,move_distance=float(move),move_direction=float(wrap(direction)),turn_angle=float(wrap(turn)),spawn_agent=spawn)
