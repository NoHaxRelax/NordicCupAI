...
def run(*, policy_spec="smoke_policy:SmokePolicy", map_seed=5101,
    ...
    for tick in range(ticks_requested):
        ...
        if mode == "oracle_approach_child_guide" and oracle_handoff_time is None:
            if guide_predator_observations:
                oracle_handoff_time = round(env.time, 1)
            elif guide.agent_id in env.agents_dict:
        ...
        for predator in tracked_predators:
            ...
            following = (not predator.resting and
                         chosen == guide.agent_id and chase_gate)
    ...
    result = json_value({
        ...
        "tracked_followed_guide": sum(followed_guide.values()),
