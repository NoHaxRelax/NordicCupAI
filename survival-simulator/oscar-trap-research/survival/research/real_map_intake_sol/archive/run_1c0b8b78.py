...
def run(*, policy_spec="smoke_policy:SmokePolicy", map_seed=5101,
    ...
    recorder = ReplayRecorder(
        ...
        notes=(__doc__ + " Replay interval is exactly one native 0.1-second tick. "
               ...
               + (" Encounter-child mode: initial agent 1 is a random-start "
                  ...
                  "parent remains the guide and the newborn is a reserve gatherer."
    ...
    result = json_value({
        ...
        "active_guide_id_final": getattr(policy, "active_guide_id", guide.agent_id),
        "active_guide_alive_final": getattr(policy, "active_guide_id",
                                             guide.agent_id) in env.agents_dict,
        "guide_role_ids": sorted(getattr(policy, "guide_role_ids", {guide.agent_id})),
        ...
        "first_predator_following_active_guide": first_follow,
