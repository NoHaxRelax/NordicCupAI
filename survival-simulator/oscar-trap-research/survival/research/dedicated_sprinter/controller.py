...
def chase(p,heading,agent,face,speed):
    ...
    nextp=p+delta
    ideal_delta=15.*np.stack([np.cos(heading+md),np.sin(heading+md)],axis=-1)
    pivotturn=np.arctan2(diff[...,1]-ideal_delta[...,1],diff[...,0]-ideal_delta[...,0])-heading
    nh=np.where(speed==0,heading,heading+np.where(direct,turn,wrap(pivotturn)))
    return nextp,nh
...
class Tracker:
    ...
    def update(self,s,t):
        ...
        for typ,point in current:
            if typ=='Fruit':continue # Unlabelled, growing/disappearing food is not a reliable pose landmark.
...
class BaitController:
    def __init__(self, *, food=True, margin=24., fuel_weight=1.4, orbit_weight=10., rest=True, center='auto', center_weight=.018, food_weight=.40, quiet=False):
        self.track=Tracker();self.food=food;self.margin=margin
        self.fuel_weight=fuel_weight;self.orbit_weight=orbit_weight;self.rest=rest;self.center=center;self.center_weight=center_weight;self.food_weight=food_weight;self.quiet=quiet
    ...
    def step(self,s,t):
        ...
        if p is None:
            ...
            lengths=np.unique(np.clip([0.,3.,5.,7.,9.,walk,11.,12.,13.,15.,16.,18.,cap],0,cap))
            ...
            gaps=[];finals=[];heads=[];pre_gaps=[]
            for sp in speeds:
                q,h=chase(p['pos'],p['heading'],tr.xy,tr.theta,np.array(sp))
                pre_gaps.append(np.linalg.norm(endpoints-q,axis=1))
            ...
            if self.center is True or (self.center=='auto' and (foodtarget is None or s['energy']>max(150.,.7*s['max_energy']))):
                # A predator turning .3 radians every step has discrete circular
                # radius v/(2 sin(.15)). Reach its interior, then hold cheaply.
                sp=max(11.,nominal) if multiplier==1 else max(3.3,nominal)
                q,h=chase(p['pos'],p['heading'],tr.xy,tr.theta,np.array(nominal))
                radial=wrap(math.atan2(tr.xy[1]-q[1],tr.xy[0]-q[0])-h)
                sign=1 if radial>=0 else -1
                center=q+sp/(2*math.sin(.15))*unit(h+sign*(math.pi/2+.15))
                cost+=self.center_weight*np.linalg.norm(endpoints-center,axis=1)**2
            else:
                cost+=self.orbit_weight*np.cos(wrap(bearing-heads[0]))
            cost+=np.maximum(0,np.max(pre_gaps,axis=0)-58)**2*3.
            if foodtarget is not None:
                progress=np.linalg.norm(endpoints-foodtarget,axis=1)-np.linalg.norm(tr.xy-foodtarget)
                cost+=self.food_weight*progress
            ...
            if self.quiet and p['raw']['distance']<48:turn=0.
            elif self.quiet and abs(turn)<s['vision_angle']/2-.15:turn=0.
