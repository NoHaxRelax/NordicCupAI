...
class SimpleChase(RealMapGuidePolicy):
    ...
    def act(self, observations, sim_time):
        ...
        for state in observations:
            ...
            # Build extra lead before crossing from the current biome into a
            # slower one. The native engine scales later movement at the new
            # biome while the follower may remain on faster terrain.
            current_modifier=self._terrain_modifier(pose.p)
            nominal=self._project_endpoint(state,pose,desired,True)
            if (nearest and nearest['distance']<65.
            ...
            for cap in (3.,6.,10.,15.,20.):
                ...
                for k in range(24):
                    ...
                    # Ordinary spacing candidates must obey the same preventive
                    # slow-terrain gate as the route endpoint.
                    if (nearest and nearest['distance']<65.
