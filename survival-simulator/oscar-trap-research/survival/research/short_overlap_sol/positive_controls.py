"""Matched long-wall positive controls for the short-overlap fixture."""
...
CASES = [
    dict(length=overlap, right_length=overlap, face_offset=0, gap=gap, seed=751)
    for overlap in (55., 65., 70., 90.) for gap in (11., 15., 19.)
]
...
if __name__ == "__main__":
    ...
    (OUT / "positive-controls-summary.json").write_text(json.dumps({
        "schema": "short-overlap-positive-controls-v1",
        "base_fixture_sha256": BASE_SOURCE_SHA256,
        "selection": "matched overlaps 55/65/70/90 x gaps 11/15/19",
        "rows": rows,
    }, indent=2) + "\n")
