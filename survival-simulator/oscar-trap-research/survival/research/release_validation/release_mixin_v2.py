            ...
            if pose is not None:
                ...
                if self.released:
                    ...
                    if self.retreat_target is None:
                        inside=sum((pose.p[i]-self.site['mouth'][i])*self.site['inward'][i] for i in range(2))
                        self.retreat_target=self.runup
                        if inside>0:
                            # A surviving guide may already have passed through
                            # the slot. Leave by the rear, never back toward the mass.
                            for distance in (self.site['overlap']+125,self.site['overlap']+90,self.site['overlap']+60):
                                q=tuple(self.site['mouth'][i]+self.site['inward'][i]*distance for i in range(2))
                                if self._free(q,5.01) and self._path(pose.p,q,5.01):
                                    self.retreat_target=q;break
