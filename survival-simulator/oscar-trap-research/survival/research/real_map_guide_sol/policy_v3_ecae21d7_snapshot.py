...
class RealMapGuidePolicy:
    ...
    def _evade(self,aid,state,pose,predator,rule):
            ...
            if self.phase=="route_to_refuge":
                if nearest is None:
                    ...
                    if sim_time-self.last_predator_seen>8:
                        self.engaged=False;self.phase="search";self.route=[]
                        self.last_predator_point=None
                        actions.append(self._move(aid,state,pose,pose.p,"acquisition_timed_out_resume_search",turn=.35,sprint=False));continue
                ...
                if nearest["distance"]<42:
                    actions.append(self._evade(aid,state,pose,predator_point,"restore_safe_follower_spacing"));continue
                ...
                if math.dist(pose.p,tuple(self.site["far"]))<6:
                    ...
                    if dot(travel,away)<-.2:
                        actions.append(self._evade(aid,state,pose,predator_point,"avoid_route_step_toward_follower"));continue
