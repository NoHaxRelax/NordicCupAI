...
class RealMapGuidePolicy:
    ...
    def __init__(self,static_map,bait_id=0,guide_id=1):
        ...
        self.last_guide_switch_tick=-10_000
        self.evade_headings={}
    ...
    def _switch_active_guide(self,agent_id,sim_time,reason):
        ...
        self.last_follow_predator_point=None;self.last_follow_guide_point=None
        self.missing_ticks=0;self.missed_gazes=0
        self.last_guide_switch_tick=self.act_tick
    ...
    def _lineage_reserve(self,aid,state):
        ...
        if pose is not None:
            if float(q["distance"])<65.:
                return self._evade(aid,state,pose,pose.point(q),
                    "lineage_reserve_immediate_latency_safe_escape",turn=q["angle"])
            return self._move(aid,state,pose,pose.p,
                "lineage_reserve_hold_for_target_reassignment",turn=q["angle"],sprint=False)
        self.decisions[aid]={"rule":"lineage_reserve_unlocalized_escape"}
        return dict(agent_id=aid,move_distance=state["sprint_speed"],
                    move_direction=wrap(q["angle"]+math.pi),
    ...
    def _move(self,aid,state,pose,target,rule,turn=0.,sprint=True):
        ...
        endpoint=add(pose.p,rot((distance*modifier,0.),pose.theta+direction))
        # Never integrate a command that the static map says crosses solid
        # geometry. The engine can rotate a colliding move, which otherwise
        # makes odometry enter the wall and leaves every later route invalid.
        if distance>0 and not self._clear(pose.p,endpoint,5.01):
            endpoint=self._local_progress_target(state,pose,target,sprint=sprint)
            delta=sub(endpoint,pose.p)
            distance=min(cap,norm(delta)/modifier)
            direction=(wrap(math.atan2(delta[1],delta[0])-pose.theta)
                       if norm(delta)>.01 else 0.)
        pose.p=endpoint;pose.theta=wrap(pose.theta+turn)
    ...
    def _orbit_target(self,pose,predator,desired):
        """Keep an unconfirmed/resting follower near while making progress."""
        # Stay inside the predator's native 60-unit hearing radius so a
        # resting predator reacquires the guide immediately on waking. A
        # 54--58 endpoint still exceeds capture after two worst-case 15-unit
        # moves (the cached-observation latency and the upcoming step).
        radial=unit(sub(pose.p,predator));radius=min(58.,max(54.,math.dist(pose.p,predator)))
        options=[add(predator,mul(rot(radial,side*.28),radius)) for side in (-1.,1.)]
        clear=[q for q in options if self._clear(pose.p,q,5.01)]
        return min(clear or options,key=lambda q:math.dist(q,desired))

    def _advance_route(self,pose,route,radius):
        """Discard a corner only after reaching it or proving the shortcut."""
        while route:
            distance=math.dist(pose.p,route[0])
            if distance<1.25:
                route.pop(0);continue
            if (distance<6 and len(route)>1 and
                    self._clear(pose.p,route[1],radius)):
                route.pop(0);continue
            break
    ...
    def _evade(self,aid,state,pose,predator,rule,turn=None,desired=None,uncertainty=0.):
        ...
        # Use an absolute latency-safe floor. Requiring a 25-unit increase
        # made the safe set mathematically empty because an agent can move at
        # most 20 units (and only 10 in swamp).
        # 15 capture radius + one already-unseen 15-unit predator move + one
        # upcoming 15-unit move + 5 margin. Unlike the old 75-unit floor,
        # this is reachable from the observed 50--65 range in one agent step.
        threshold=50.+uncertainty
        safe=[q for q in candidates if math.dist(q,predator)>=threshold]
        previous=self.evade_headings.get(aid)
        def commitment(q):
            return (dot(unit(sub(q,pose.p)),previous) if previous is not None else 0.)
        if safe:
            desired=desired or tuple(self.site["far"])
            # Once spacing is safe, prefer tangential progress over fleeing to
            # the arena boundary. Retaining the prior escape heading prevents
            # north/south flips beside concave obstacle corners.
            target=max(safe,key=lambda q:(math.dist(pose.p,desired)-math.dist(q,desired)
                                          +.02*math.dist(q,predator)+2.*commitment(q)))
        else:
            desired=desired or tuple(self.site["far"])
            target=max(candidates,key=lambda q:(math.dist(q,predator)
                       +.25*(math.dist(pose.p,desired)-math.dist(q,desired))
                       +2.*commitment(q)),default=pose.p)
        if math.dist(target,pose.p)>.01:self.evade_headings[aid]=unit(sub(target,pose.p))
        if turn is None:
    ...
    def act(self,observations,sim_time):
        ...
        if candidate is not None and candidate!=self.active_guide_id:
            ...
            # Four consistent native frames plus a one-second cooldown prevent
            # the parent/child role thrash seen in v12. Both lineage members
            # evade at unsafe range during this evidence window.
            if self.pending_guide_ticks>=4 and self.act_tick-self.last_guide_switch_tick>=10:
                self._switch_active_guide(candidate,sim_time,"four_native_facing_closest_frames")
                self.pending_guide_candidate=None;self.pending_guide_ticks=0
        ...
        for aid,state in sorted(states.items()):
            if aid==self.bait_id:
                ...
                self._advance_route(pose,self.bait_route,5.01)
            ...
            if self.phase=="route_to_refuge":
                if nearest is None:
                    ...
                    if not self.follow_confirmed and self.last_predator_point is not None:
                        # A native predator regularly rests and drops its
                        # target. Stay in a 54--58 unit orbit so its native
                        # hearing guarantees acquisition when it wakes.
                        # Looking ticks retain
                        # motion; intervening ticks turn beyond perpendicular.
                        bearing=wrap(math.atan2(self.last_predator_point[1]-pose.p[1],
                                                self.last_predator_point[0]-pose.p[0])-pose.theta)
                        turn=bearing if scheduled_gaze else wrap(bearing+math.pi/2+.12)
                        remembered=math.dist(pose.p,self.last_predator_point)
                        if remembered<60:
                            actions.append(self._evade(aid,state,pose,self.last_predator_point,
                                "unconfirmed_reacquisition_safe_escape",turn=turn,
                                desired=tuple(self.site["far"])));continue
                        if remembered>82:
                            target=self._local_progress_target(state,pose,self.last_predator_point)
                            actions.append(self._move(aid,state,pose,target,
                                "unconfirmed_reacquisition_close_range",turn=turn,sprint=False));continue
                        target=self._orbit_target(pose,self.last_predator_point,tuple(self.site["far"]))
                        actions.append(self._move(aid,state,pose,target,
                            "unconfirmed_reacquisition_orbit",turn=turn,sprint=False));continue
                    if scheduled_gaze and self.last_predator_point is not None:
                        ...
                        endpoint=self._project_endpoint(state,pose,desired,sprint=False)
                        post_move_bound=(math.dist(endpoint,self.last_predator_point)
                                         -15*(self.missing_ticks+2))
                        if post_move_bound<30:
                            actions.append(self._evade(aid,state,pose,self.last_predator_point,
                                "scheduled_gaze_moving_latency_safe_tangent",turn=turn,
                                desired=desired,uncertainty=15*self.missing_ticks));continue
                        actions.append(self._move(aid,state,pose,desired,
                            "scheduled_gaze_while_continuing_route",turn=turn,sprint=False));continue
                ...
                if not self.follow_confirmed:
                    ...
                    target=self._orbit_target(pose,predator_point,tuple(self.site["far"]))
                    actions.append(self._move(aid,state,pose,target,
                        "cadenced_moving_orbit_verify_following",turn=gaze_turn,sprint=False));continue
                if nearest["distance"]>90:
                    actions.append(self._move(aid,state,pose,predator_point,
