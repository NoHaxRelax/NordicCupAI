...
class RealMapGuidePolicy:
    ...
    def _evade(self,aid,state,pose,predator,rule,turn=None):
        """Choose a locally clear sprint endpoint maximizing separation."""
    ...
    def _observe_following(self,nearest,sim_time):
        ...
        closing=(self.last_follow_distance is not None and
                 distance<=self.last_follow_distance+1.)
        if facing and (closing or distance<=55):self.follow_score=min(4,self.follow_score+1)
        ...
        self.follow_history.append(dict(time=round(sim_time,2),distance=round(distance,2),
                                        facing=facing,closing=closing,score=self.follow_score))
    ...
    def act(self,observations,sim_time):
        ...
        for aid,state in sorted(states.items()):
            if aid==self.bait_id:
                ...
                if measured is not None and math.dist(pose.p,goal)>6:self.bait_ready=False
                if self.bait_ready:
                    actions.append(self._move(aid,state,pose,goal,"hold_deployed_bait_in_refuge",sprint=False));continue
            ...
            if aid!=self.guide_id:
                actions.append(self._forage_unmapped(aid,state));continue
            measured=(self._localize(state) if aid not in self.poses or
                      self._local_edges(state) else None)
            if measured:self.poses[aid]=measured
            pose=self.poses.get(aid)
            ...
            if nearest:self._observe_following(nearest,sim_time)
            ...
            if self.phase=="route_to_refuge":
                ...
                if nearest["distance"]<42:
                    actions.append(self._evade(aid,state,pose,predator_point,"restore_safe_follower_spacing",turn=gaze_turn));continue
                ...
                if math.dist(pose.p,tuple(self.site["far"]))<6:
                    ...
                    if dot(travel,away)<-.2:
                        actions.append(self._evade(aid,state,pose,predator_point,"avoid_route_step_toward_follower",turn=gaze_turn));continue
