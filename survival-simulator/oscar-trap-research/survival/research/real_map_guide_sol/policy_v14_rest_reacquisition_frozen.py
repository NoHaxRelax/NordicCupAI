...
class RealMapGuidePolicy:
    ...
    def _orbit_target(self,pose,predator,desired):
        ...
        radial=unit(sub(pose.p,predator));radius=min(76.,max(68.,math.dist(pose.p,predator)))
            ...
            if self.phase=="route_to_refuge":
                if nearest is None:
                    ...
                    if not self.follow_confirmed and self.last_predator_point is not None:
                        ...
                        # target. Stay in a 68--76 unit orbit so it can acquire
                        # the guide again when it wakes. Looking ticks retain
