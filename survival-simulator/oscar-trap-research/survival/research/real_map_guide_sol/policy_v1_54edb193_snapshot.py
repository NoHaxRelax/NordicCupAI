...
class RealMapGuidePolicy:
    ...
    def __init__(self,static_map,bait_id=0,guide_id=1):
        ...
        self.last_predator_seen=-1e9;self.engaged=False;self.events=[];self.decisions={}
    ...
    def _path(self,start,target,radius=11.0):
        if self._clear(start,target,radius):return [target]
        ...
        while heap:
            ...
            for j in range(1,len(nodes)):
                ...
                if cost+d>=dist[j] or not self._clear(nodes[i],nodes[j],radius):continue
    ...
    def act(self,observations,sim_time):
        ...
        for aid,state in sorted(states.items()):
            ...
            if self.phase=="route_to_refuge":
                if nearest is None and sim_time-self.last_predator_seen>.6:
                    actions.append(self._move(aid,state,pose,pose.p,"wait_for_follower_reobservation",turn=.35,sprint=False));continue
                if nearest and nearest["distance"]>72:
                ...
                if not self.route or math.dist(pose.p,tuple(self.site["far"]))<6:
                    self.phase="wait_aligned_follower";self.route=[]
                else:
                    actions.append(self._move(aid,state,pose,self.route[0],"obstacle_aware_follower_route",turn=face_away,sprint=True));continue
