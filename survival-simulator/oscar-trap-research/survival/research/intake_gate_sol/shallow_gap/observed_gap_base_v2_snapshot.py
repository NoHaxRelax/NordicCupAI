...
walks to a point 15 units inside the nearest mouth, faces into the passage, and
    ...
    def _identify(self, group, states):
        ...
        for i, first in enumerate(group.edges):
            ...
            for second in group.edges[i + 1:]:
                ...
                if not 54.9 <= other_length <= 100.1:
                    continue
                ...
                if not 10.9 <= gap <= 19.1:
                    continue
                first_projection = sorted(dot(point, tangent) for point in first)
                second_projection = sorted(dot(point, tangent) for point in second)
