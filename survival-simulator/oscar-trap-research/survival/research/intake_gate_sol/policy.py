...
class IntakeGatePolicy(WallPolicy):
    ...
    def _update_gate(self, station, states):
        ...
        for observer in station['holders']:
            ...
            for obs in states[observer]['observations']:
                ...
                raw_diag.append((observer, obs['distance'],
                    dot(sub(point, station['anchor']), station['normal'])))
                ...
                local.append((point, heading))
            # Preserve all same-observer entries as a multiset.  Only suppress
            # a one-to-one geometric match from an additional bait.
            unmatched = list(range(len(points)))
            for point, heading in local:
                match = next((i for i in unmatched
                              if math.dist(points[i][0], point) < .05), None)
                if match is None:
                    points.append((point, heading))
                else:
                    unmatched.remove(match)
        current = points
        if raw_diag and not station.get('reported_predator_sensor'):
            station['reported_predator_sensor'] = True
            self._event('predator_sensor_online', samples=raw_diag)
    ...
    def _temporary_target(self, pose, state, face, outward, worker):
        """Orbit the arriving predator while staying >=90 from the wall face."""
        ...
        point = pose.polar(nearest)
        away = unit(sub(pose.p, point))
        sign = worker.setdefault('orbit_sign', 1)
        side = mul((-away[1], away[0]), sign)
        radial = 35-nearest['distance']
        goal = add(pose.p, add(mul(side, state['sprint_speed']), mul(away, radial*.6)))
        clearance = dot(sub(goal, face), outward)
        if clearance < 90:
            goal = add(goal, mul(outward, 90-clearance))
        return goal, threats
        ...
        for group in self.groups:
            ...
            for aid in live:
                ...
                if station and aid in station['holders']:
                    ...
                    face = add(station['front'], mul(station['normal'], 5.1))
                    n, tangent = station['normal'], station['tangent']
                    outward = mul(n, -1)
                    stage = add(face, mul(outward, 90))
                    threats = [o for o in state['observations'] if o['type'] == 'Predator']
                    if threats:
                        worker['saw_threat'] = True
                    if worker['phase'] == 'approach':
                        target, speed, rule = stage, state['sprint_speed'], 'approach_temporary_holder'
                        if math.dist(pose.p, target) < 2:
                            worker['phase'] = 'hold'
                    if worker['phase'] == 'hold':
                        if worker.get('saw_threat') and (held_seen == 0 or sleep_ready):
                            worker['phase'] = 'deliver'
                            self._event('sleep_gated_commit', guide=aid, observed_old=held_seen)
                        else:
                            target, threats = self._temporary_target(pose, state, face, outward, worker)
                            speed, rule = state['sprint_speed'], 'temporary_holder_rectangle'
                    if worker['phase'] == 'deliver':
                        target, speed, rule = station['front'], state['sprint_speed'], 'sleep_window_final_sprint'
                    if threats:
                        nearest = min(threats, key=lambda o:o['distance'])
                    ...
                    if len(live) == 1 and edges:
                        ...
                        if worker['phase'] == 'hold':
                            ...
                            if worker.get('saw_threat') and (held_seen == 0 or sleep_ready):
                                ...
                                target, threats = self._temporary_target(pose, state, mid, normal, worker)
                                speed = state['sprint_speed']
