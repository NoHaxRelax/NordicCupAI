...
class ObservedFunnel(_base.ObservedFunnel):
    ...
    def __init__(self, capacity=33, gate=True, replenish=False, gather=False,
                 orbit_radius=44.0, still_delay=0.25, fresh_rest_limit=1.4):
    ...
    @staticmethod
    def _long_edge(group, pose):
        edges = [e for e in group.edges if 69.9 <= math.dist(*e) <= 100.1]
        return min(edges, key=lambda e:math.dist(pose.p, _base.mul(_base.add(*e), .5))) if edges else None
    ...
    def act(self, observations, sim_time):
        ...
        for i, action in enumerate(actions):
            ...
            if id(group) in self.stations:
                continue
            edge = self._long_edge(group, self.poses[aid])
            if edge is None:
                continue
            pose = self.poses[aid]
