...
from observed_gap_guide_policy_v3 import ObservedGapGuidePolicy
...
POLICY_HASH=hashlib.sha256((HERE/"observed_gap_guide_policy_v3.py").read_bytes()+
...
def run(predators=4,interval=90.,seconds=400.,seed=4301,gap=15.,length=90.,
    ...
    policy=ObservedGapGuidePolicy();tag=f"observed-gap-guides-v3-hold-g{gap:g}-l{length:g}x{right_length:g}-o{face_offset:g}-h{int(horizontal)}-n{predators}-i{interval:g}-lat{lateral:g}-s{seed}-{uuid4().hex[:8]}"
    replay=OUT/"replays"/f"{tag}.json.gz"
    rec=ReplayRecorder(env,title=tag,policy="observation-only-far-mouth-gap-guides-v3",seed=seed,every=every,
