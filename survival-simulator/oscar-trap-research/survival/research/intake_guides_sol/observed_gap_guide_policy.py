"""Observation-only fast, centered entry into a predator-excluding gap.

Each disconnected supplied guide maps the two inner faces itself. It first
sprints to a centered point 18 units outside the observed mouth, then enters
straight along the passage axis. Fixture geometry is never supplied to the
controller.
"""
...
BASE=Path(__file__).with_name("observed_gap_base_snapshot.py")
    ...
    def act(self,observations,sim_time):
        ...
        for group in self.groups:
            ...
            for aid in live:
                ...
                if station:
                    outside=base.sub(station["mouth"],base.mul(station["inward"],18.))
                    phase=self.phases.setdefault(aid,"center_on_mouth")
                    if phase=="center_on_mouth" and (math.dist(pose.p,outside)<4 or
                       base.dot(base.sub(pose.p,outside),station["inward"])>=0):
