...
class ObservedFunnel(_base.ObservedFunnel):
    ...
    def __init__(self, capacity=33, gate=True, replenish=False, gather=False,
                 orbit_radius=34.0, still_delay=0.25):
    ...
    def _safe_station(self, sim_time):
        return next((s for s in self.stations.values()
                     if s.get("still_since") is not None
                     and sim_time - s["still_since"] >= self.still_delay), None)
