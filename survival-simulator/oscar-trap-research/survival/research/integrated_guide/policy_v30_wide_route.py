"""V29 single guide with static terrain costs and wider route clearance.

Route-only wide navigation reduces sharp pursuit corners. Final intake retains
its existing geometry; all dynamic decisions still use native observations.
"""
import math,heapq
from short_overlap_sol.policy_v29_terrain_emergency import Guide as Base
from integrated_guide.spatial_geometry import SpatialGeometry

class Guide(SpatialGeometry, Base):
    GRID=20.
    ...
    def _path(self,start,target,radius=11.):
        if radius>=10.9 and getattr(self,'phase','route') in ('localize','route'):
            for preferred in (35.,25.):
                if self._free(target,preferred):
                    route=self._terrain_path(start,target,preferred)
                    if route:return route
        return self._terrain_path(start,target,radius) or super()._path(start,target,radius)

    def _terrain_path(self,start,target,radius=11.):
        ...
        if not near_start or not near_end:return []
