"""Experimental compact approach selector; no native reliability evidence yet.

Derived from immutable selector.py. Keeps depth5 and opposite-mouth access,
but offers 100/60 approach staging, a shorter runup, and gaps up to19.9.
Do not pool its geometry or future tests with the frozen128 census.
"""
...
def enumerate_sites(static_map: Mapping, *, min_gap: float = 10.1,
                    max_gap: float = 19.9, min_overlap: float = 10.3,
                    bait_depth: float = 5.0,
                    far_distance: float = 100., hold_distance: float = 60.,
                    rear_staging_distance: float = 10.01,
            ...
            for axis in (0, 1):
                ...
                for sign in (1, -1):
                    ...
                    for offset in offsets:
                        ...
                        far = shifted(add_depth(mouth, -far_distance))
                        hold = shifted(add_depth(mouth, -hold_distance))
                        if not (g.free(far, 11.01) and g.free(hold, 11.01)
                                and g.clear(far, hold, 11.01)):
                            continue
                        runups = [shifted(add_depth(mouth, -d))
                                  for d in (200., 175., 150., 130., 120., 110.)]
                    ...
                    second_staging = add_depth(other_mouth, rear_staging_distance)
                    ...
                    site = dict(
                        ...
                        experimental_compact_approach=True,
                        approach_far_distance=far_distance, approach_hold_distance=hold_distance,
                        rear_staging_distance=rear_staging_distance,
                    )
