...
from observed_gap_guide_policy_v4 import ObservedGapGuidePolicy
...
OUT=ROOT/"results"/"depth5_test"/"guided"
POLICY_HASH=hashlib.sha256((HERE/"observed_gap_guide_policy_v4.py").read_bytes()+
...
def run(predators=4,interval=90.,seconds=400.,seed=4301,gap=15.,length=90.,
    ...
    policy=ObservedGapGuidePolicy();tag=f"depth5-observed-gap-guides-v4-aligned-hold-g{gap:g}-l{length:g}x{right_length:g}-o{face_offset:g}-h{int(horizontal)}-n{predators}-i{interval:g}-lat{lateral:g}-s{seed}-{uuid4().hex[:8]}"
    replay=OUT/"replays"/f"{tag}.json.gz"
    rec=ReplayRecorder(env,title=tag,policy="observation-only-aligned-far-mouth-guides-v4",seed=seed,every=every,
        scenario="arranged size-selective gap and sequential off-axis guided predator arrivals",
        notes="Bait depth5 inside observed mouth; every0.1-second frame retained with native rendering. "+__doc__+" Living agents are refilled before every tick; native predator movement, rest, sensing, collision, capture and target selection remain unchanged.",
    ...
    for tick in range(round(seconds*10)):
        ...
        if idx<predators and env.time+.001>=due:
            ...
            # ReplayRecorder extracts births before applying its same-time frame
            # filter.  Capture now so a same-tick guide death remains auditable.
            rec.capture(force=True)
