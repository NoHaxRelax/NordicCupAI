...
from streaming_recorder_v2 import ReplayRecorder as Streaming
...
assert not b.frames.path.exists() and not b.events.path.exists()
assert y['events'], 'event comparison must not be empty'
report={'passed':True,'same_native_steps':10,'frames':11,'exact_match':['world','frames','events','summary'],
        ...
        'streaming_source_sha256':hashlib.sha256((ROOT/'debugger/streaming_recorder_v2.py').read_bytes()).hexdigest()}
(ROOT/'results/resource_validation/equivalence-v2.json').write_text(json.dumps(report,indent=2)+'\n')
