...
class ObservedFunnel(WallPolicy):
    def __init__(self, capacity=33, gate=True):
        super().__init__()
        ...
        for edge in group.edges:
            ...
            for other in group.edges:
                ...
                if not 29.9 <= width <= 35.1 or abs(dot(delta,tangent))>.1:
                    continue
                return dict(holder=holder,anchor=hp,front=add(om,mul(normal,-7)),
        ...
        for group in self.groups:
            ...
            for aid in live:
                ...
                if not station:
                    ...
                    spawn=len(live)==1 and station['seen']<self.capacity and state['energy']>200
                else:
