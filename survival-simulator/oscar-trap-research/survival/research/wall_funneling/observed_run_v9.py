...
def run(width=30,length=100,predators=3,spread=15.,seconds=180.,seed=41,
        ...
        replenish=False,depth_spread=40.,waves=1,interval=20.,gather=False,baits=1,initial_jitter=0.):
    ...
    for i in range(stations):
        ...
        for j in range(count):
            ...
            if when==0:
                ...
                scheduled.append((when,px,py,heading))
    ...
    policy=ObservedFunnel(capacity=math.ceil(predators/stations),gate=gate,replenish=replenish,gather=gather)
    ...
    rec=ReplayRecorder(env,title=tag,policy='observation-only-funnel-v9',seed=seed,every=10,
