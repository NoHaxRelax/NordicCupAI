...
from observed_policy_experimental import ObservedFunnel as StagedGuideFunnel
...
EXPERIMENTAL_HASH=hashlib.sha256(Path(__file__).with_name('observed_policy_experimental.py').read_bytes()).hexdigest()
...
def run(width=30,length=100,predators=3,spread=15.,seconds=180.,seed=41,
        ...
        replenish=False,depth_spread=40.,waves=1,interval=20.,gather=False,baits=1,initial_jitter=0.,staged_guides=False):
    ...
    for i in range(stations):
        ...
        offsets={1:[0],2:[-10,10],3:[0,-10,10]}[baits]
        holders=[]
        for offset in offsets:
            anchor=point(width/2+5.1,offset)
            initial=point(width/2+5.1+rng.uniform(0,min(4,initial_jitter/2)),
                offset+rng.uniform(-initial_jitter,initial_jitter)) if initial_jitter else anchor
            holder=add_agent(env,x=initial[0],y=initial[1],energy=150);holder.direction=math.atan2(-n[1],-n[0])
            holders.append((holder,anchor))
        ...
        for j in range(count):
            ...
            heading=math.atan2(n[1],n[0])+rng.uniform(-.3,.3)
            when=(j%waves)*interval
            if when==0:
                p=add_predator(env,px,py,heading=heading,energy=102 if awake else 0)
                p.resting=not awake;ps.append(p)
            else:
                guide_point=point(-width/2-125)
                scheduled.append((when,px,py,heading,guide_point,math.atan2(n[1],n[0])))
        fixtures.append(dict(wall=wall,holders=holders,n=n,t=t,predators=ps,center=(cx,cy)))
    env.edges={tuple(sorted((a,b))) for o in env.obstacles for a,b in o.edges}
    env._update_spatial_grid()
    for creature in env.agents+env.predators:
        assert not env._in_obstacle((creature.x,creature.y),radius=creature.size,obstacles=env.obstacles)
    controller_class=StagedGuideFunnel if staged_guides else ObservedFunnel
    policy_hash=EXPERIMENTAL_HASH if staged_guides else POLICY_HASH
    policy=controller_class(capacity=math.ceil(predators/stations),gate=gate,replenish=replenish,gather=gather)
    tag=f'observed-w{width}-l{length}-n{predators}-sites{stations}-s{seed}-spread{spread}-waves{waves}-gate{int(gate)}-h{int(horizontal)}-a{int(awake)}-{uuid4().hex[:8]}'
    path=OUT/'replays'/f'{tag}.json.gz'
    rec=ReplayRecorder(env,title=tag,policy='observation-staged-guides-v10' if staged_guides else 'observation-only-funnel-v9',seed=seed,every=10,
        scenario='arranged walls and predator approaches; observation-only policy',
        notes=__doc__+(' Later guides are supplied by the fixture at prepared approach points; recruitment is not simulated.' if staged_guides else ''),native_render=native,native_width=800,
    ...
    for tick in range(round(seconds*10)):
        if not env.agents:
            break
        due=[item for item in scheduled if item[0]<=env.time+.001]
        for item in due:
            when,px,py,heading,guide_point,guide_heading=item
            if staged_guides and (when,guide_point) not in supplied_guides:
                aid=env._next_agent_id
                guide=add_agent(env,*guide_point,energy=150)
                guide.agent_id=aid;guide.direction=guide_heading
                env._next_agent_id+=1
                env.agents_dict={a.agent_id:a for a in env.agents}
                supplied_guides.add((when,guide_point))
            p=add_predator(env,px,py,heading=heading,energy=102 if awake else 0)
            p.resting=not awake;streaks[p]=0
            scheduled.remove(item)
        if due and staged_guides:
            states=[ObservationResponse(**env.get_agent_state(a.agent_id)).model_dump() for a in env.agents]
        ...
        deaths.extend(dict(id=a.agent_id,time=round(env.time,1),holder=any(a is h for f in fixtures for h,_ in f['holders']))
        ...
        for p in env.predators:
            ...
            for f in fixtures:
                n=f['n'];t=f['t']
                tangent=(p.x-f['center'][0])*t[0]+(p.y-f['center'][1])*t[1]
                ...
                for holder,anchor in f['holders']:
                    anchor=(holder.x,holder.y)
                    clearance=(holder.x-f['center'][0])*n[0]+(holder.y-f['center'][1])*n[1]-width/2
                    delta=(p.x-anchor[0],p.y-anchor[1])
                    normal=delta[0]*n[0]+delta[1]*n[1]
                    valid=(holder.agent_id in env.agents_dict and 5-.001<=clearance<=8 and math.hypot(*delta)<=60
    ...
    row=dict(width=width,length=length,predators=predators,spread=spread,seconds=round(env.time,1),
        ...
        waves=waves,interval=interval,arrivals=len(env.predators),gather=gather,baits=baits,initial_jitter=initial_jitter,
        staged_guides=staged_guides,supplied_guides=len(supplied_guides),
        ...
        holders_alive=sum(h.agent_id in env.agents_dict for f in fixtures for h,_ in f['holders']),
...
if __name__=='__main__':
    ...
    p.add_argument('--baits',type=int,choices=[1,2,3],default=1)
    p.add_argument('--initial-jitter',type=float,default=0)
    p.add_argument('--staged-guides',action='store_true',help='Fixture supplies an approaching guide with each later wave; not autonomous recruitment')
