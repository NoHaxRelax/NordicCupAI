    ...
    def identify(self, group, states):
        ...
        for edge in group.edges:
            ...
            for other in group.edges:
                ...
                offsets={1:[0],2:[-10,10],3:[-10,0,10]}.get(len(protected),[0]*len(protected))
                holders={aid:add(add(midpoint,mul(normal,5.1)),mul(tangent,offset))
                         for aid,offset in zip(protected,offsets)}
                return dict(holder=holder,holders=holders,anchor=holders[holder],front=add(om,mul(normal,-5.1)),
                    normal=normal,tangent=tangent,length=length,width=width,
                    seen=0,previous=[],still_since=None,gate_until=-1.,mapped=self.time,
                    last_worker_seen=self.time)
        return None
        ...
        for group in self.groups:
            ...
            if station and station['holder'] in states:
                ...
                for observer in station['holders']:
                    if observer not in states:continue
                    op=self.poses[observer]
                    local=[op.polar(o) for o in states[observer]['observations']
                        if o['type']=='Predator' and
                        dot(sub(op.polar(o),station['anchor']),station['normal']) < -20]
                    # Multiset union preserves separate colocated predators and
                    # avoids counting the same observation from two baits twice.
                    unmatched=list(range(len(points)))
                    for point in local:
                        match=next((i for i in unmatched if math.dist(points[i],point)<.05),None)
                        if match is None:points.append(point)
                        else:unmatched.remove(match)
                if any(aid not in station['holders'] for aid in live):
                    station['last_worker_seen']=sim_time
            ...
            for aid in live:
                ...
                if not station:
                    turn=.35
                    # A separately arriving guide can recognize its front face
                    # locally; it need not inherit a holder's coordinate frame.
                    edges=[edge for edge in group.edges if 69.9<=math.dist(*edge)<=100.1]
                    if len(live)==1 and edges:
                        edge=min(edges,key=lambda e:math.dist(pose.p,mul(add(*e),.5)))
                        mid=mul(add(*edge),.5);tangent=unit(sub(edge[1],edge[0]))
                        normal=(-tangent[1],tangent[0])
                        if dot(sub(pose.p,mid),normal)<0:normal=mul(normal,-1)
                        ...
                        if threats:
                            ...
                            if nearest['distance']<45:speed=state['sprint_speed']
                        if worker.get('front_commit'):
                            target=add(mid,mul(normal,5.1));rule='independent_observed_front_delivery'
                            if threats and nearest['distance']>75:target=pose.p
                            if not threats:
                                v=sub(mid,pose.p);turn=wrap(math.atan2(v[1],v[0])-pose.theta)
                elif aid in station['holders']:
                    ...
                    spawn=(self.replenish and aid==station['holder'] and
                        len(live)==len(station['holders']) and station['seen']<self.capacity and state['energy']>200
                        and sim_time-station['last_worker_seen']>2)
                else:
                    ...
                    if worker['phase']=='collect':
                        ...
                        if threats:
                            ...
                            can_deliver=(not self.gate or station['seen']==0 or
                                         (sim_time<station['gate_until'] and dot(sub(pose.p,p),n)>20))
                            if self.gather and station['seen']==0:
                                ...
                                angle=worker.setdefault('wait_angle',math.atan2(pose.p[1]-standby[1],pose.p[0]-standby[0]))
                                target=add(standby,(45*math.cos(angle),45*math.sin(angle)))
                                if math.dist(pose.p,target)<12:
                                    worker['wait_angle']=angle+.35
                                    target=add(standby,(45*math.cos(angle+.35),45*math.sin(angle+.35)))
