...
def run(width=30, length=100, mode='sacrifice', heading=0., offset=0.,
    ...
    rec = ReplayRecorder(env, title=f'Food-free funnel: {tag}', policy='wall-funneling-v2',
        ...
        'rest_gate delays introduction of the off-map pair until all existing '
        'predators rest at energy <=40; the off-map queue is not simulated.',
    ...
    for tick in range(round(seconds*10)):
        ...
        if len(deliveries) < waves and due and gate_open:
            ...
            deliveries.append(dict(guide=guide, predator=p, phase='lead', waypoints=[],
                ...
                held_ticks=0, tail=[], switches=0, physical_first_loss=None,
                active_target_loss_ticks=0))
        ...
        for d in deliveries:
            ...
            physical = (holder.agent_id in env.agents_dict
                and math.dist((p.x,p.y),anchor) <= 60
                and wall.x-55 < p.x <= wall.x-10
                and wall.y+5 < p.y < wall.y+length-5)
            if d['acquired'] is not None:
                if not physical and d['physical_first_loss'] is None:
                    d['physical_first_loss'] = round(env.time,1)
                d['active_target_loss_ticks'] += int(not p.resting and chosen != holder.agent_id)
    ...
    items = [dict(arrival=d['arrival'],acquired=d['acquired'],first_loss=d['first_loss'],
        ...
        physical_first_loss=d['physical_first_loss'],
        active_target_loss_ticks=d['active_target_loss_ticks'],
    ...
    result = dict(width=width,length=length,mode=mode,heading=heading,offset=offset,peel=peel,
        ...
        physical_continuous_success=len(items)==waves and env.time>=seconds-.01 and
            all(d['acquired'] is not None and d['physical_first_loss'] is None for d in items),
