...
def run(width=30,length=100,predators=3,spread=15.,seconds=180.,seed=41,
        ...
        replenish=True,depth_spread=40.):
    ...
    for i in range(stations):
        cx,cy=(800,600) if stations==1 else (430+(i%3)*430,230+(i//3)*350)
    ...
    rec=ReplayRecorder(env,title=tag,policy='observation-only-funnel-v1',seed=seed,every=10,
        ...
        policy_sha256=hashlib.sha256(Path(__file__).with_name('observed_policy.py').read_bytes()).hexdigest())
    rec.capture()
    ...
    row=dict(width=width,length=length,predators=predators,spread=spread,seconds=round(env.time,1),
        ...
        replenish=replenish,depth_spread=depth_spread,
...
if __name__=='__main__':
    ...
    p.add_argument('--no-renewal',dest='replenish',action='store_false')
