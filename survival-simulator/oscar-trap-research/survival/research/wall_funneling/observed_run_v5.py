...
def run(width=30,length=100,predators=3,spread=15.,seconds=180.,seed=41,
        ...
        replenish=False,depth_spread=40.,waves=1,interval=20.,gather=False):
    ...
    rec=ReplayRecorder(env,title=tag,policy='observation-only-funnel-v5',seed=seed,every=10,
    ...
    row=dict(width=width,length=length,predators=predators,spread=spread,seconds=round(env.time,1),
        ...
        waves=waves,interval=interval,arrivals=len(env.predators),gather=gather,
