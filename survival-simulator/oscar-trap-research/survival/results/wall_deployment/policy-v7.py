...
class WallPolicy:
    ...
    def _register(self, states):
        ...
        while unregistered:
            progress = False
            for child in sorted(unregistered.copy()):
                # Newborn can hear a known parent even if that parent's cached
                # observation was skipped by the native removal iteration.
                for o in states[child]['observations']:
                    if o['type']!='Agent' or o['id'] not in self.poses or o['id'] not in states: continue
                    parent=o['id']; known=self.poses[parent]
                    if o['distance']<1e-6:
                        mutual=next((v for v in states[parent]['observations'] if v['type']=='Agent'
                            and v['id']==child and v['distance']<1e-6),None)
                        if mutual is None: continue
                        # atan2(0,0) is zero in the native sensor. Each zero-gap
                        # angle is therefore minus its observer's world heading;
                        # their difference yields the correct relative heading.
                        p=known.p
                        theta=wrap(known.theta+mutual['angle']-o['angle'])
                    else:
                        bearing=known.theta+o['rel_dir']
                        p=add(known.p,mul((math.cos(bearing),math.sin(bearing)),o['distance']))
                        theta=wrap(bearing+math.pi-o['angle'])
                    self.poses[child]=Pose(p,theta)
                    self.group_for[child]=self.group_for[parent]
                    self.group_for[parent].members.add(child)
                    unregistered.remove(child); progress=True
                    break
            for aid in sorted(set(states) & set(self.poses)):
                ...
                for o in states[aid]['observations']:
                    if o['type'] == 'Agent' and o['id'] in unregistered:
                        ...
                        p = pose.polar(o)
                        if o['distance']<1e-6:
                            mutual=next((v for v in states[other]['observations'] if v['type']=='Agent'
                                and v['id']==aid and v['distance']<1e-6),None)
                            if mutual is None: continue
                            theta=wrap(pose.theta+o['angle']-mutual['angle'])
                        else:
    ...
    def _map(self, aid, state):
        pose, group = self.poses[aid], self.group_for[aid]
        raw = list(dict.fromkeys(tuple(tuple(p) for p in o['coords'])
                   for o in state['observations'] if o['type'] == 'Edge'))
        by_length = {}
        for edge in group.edges:
            by_length.setdefault(round(math.dist(*edge),3),[]).append(edge)
        ...
        for local in raw:
            ...
            for x, y in by_length.get(round(math.dist(a,b),3),[]):
                for c, d in ((x, y), (y, x)):
                    shift = sub(c, a)
        ...
        for local in raw:
            ...
            if not any((math.dist(edge[0], e[0]) < .05 and math.dist(edge[1], e[1]) < .05)
                       ...
                       for e in by_length.get(round(math.dist(*edge),3),[])):
                group.edges.append(edge)
                by_length.setdefault(round(math.dist(*edge),3),[]).append(edge)
        # There are only 336 physical edges in a native 80-obstacle arena.
        # Bound stale duplicated landmarks after prolonged unobserved drift.
        if len(group.edges)>512: group.edges=group.edges[-512:]
        seen_food = []
        ...
        for group in self.groups:
            ...
            if group.trap:
                ...
                if unsafe:
                    self._event('abandon_protected_side_intrusion',holder=trap.holder,
                        perceived_predators=predators,anchor=trap.anchor,normal=trap.normal,
                        sensed_by=[dict(agent=aid,pose=self.poses[aid].p,theta=self.poses[aid].theta,
                            observations=[o for o in states[aid]['observations'] if o['type']=='Predator'])
            ...
            for aid in sorted(live):
                ...
                spawn = (aid not in group.retired and (not trap or aid != trap.incoming)
                         ...
                         and len(states)+global_births < 18
