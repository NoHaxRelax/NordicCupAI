...
class WallPolicy:
    def __init__(self, wall=True, ripeness_wait=15., retire_age=65., active_cap=3):
        ...
        self.metrics = dict(edge_corrections=0, max_edge_correction=0.,
                            ...
                            emergency_flees=0, predicted_births=0)
    ...
    def _map(self, aid, state):
        ...
        for local in raw:
            ...
            for x, y in group.edges:
                for c, d in ((x, y), (y, x)):
                    shift = sub(c, a)
                    if norm(sub(add(b, shift), d)) < .02 and norm(shift) < 35:
    ...
    def _blocked(self, group, start, end, clearance=6.):
        ...
        return any(line.distance(LineString(e)) < clearance-1e-4 for e in group.edges)
    ...
    def _waypoint(self, group, aid, target):
        ...
        edges = [e for e in group.edges if LineString(e).distance(Point(p)) < 160]
        nodes = [p, target]
        for a, b in edges:
        ...
        while heap:
            ...
            for j in range(1, len(nodes)):
                ...
                if not self._blocked(group, nodes[i], nodes[j]):
                    heapq.heappush(heap, (cost+math.dist(nodes[i], nodes[j]), j, path+[j]))
        return p

    def _recognize(self, group, states, predators):
        if not self.wall or group.trap is not None: return
        for aid in sorted(group.members & states.keys()):
            p = self.poses[aid].p
        ...
        for tr in group.predator_tracks:
            ...
            for a, b in group.edges:
                if not 70 <= math.dist(a, b) < 250: continue
                u = unit(sub(b, a)); n = (-u[1], u[0])
                ...
                for c, d in group.edges:
                    for corner, far in ((c, d), (d, c)):
                        ...
                        if not 15 <= norm(short) <= 35 or abs(dot(unit(short), u)) > .01: continue
