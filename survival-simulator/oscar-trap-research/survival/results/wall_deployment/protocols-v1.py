...
def run(kind='first',width=30,height=100,gap=60,side=1,heading=0.,seconds=35,
    ...
    path=OUT/'replays'/f'protocol-v1-{tag}-{uuid4().hex[:10]}.json.gz'
    rec=ReplayRecorder(env,title=f'Trap protocol: {tag}',policy='privileged protocol feasibility v1',seed=1729,
        ...
        for a in sorted(before,key=lambda a:a.agent_id):
            ...
            if a is holder:
                ...
                if kind=='guard_active':
                    ...
                    if not guard_active and incoming.resting and incoming.x<guard.x:
                        target=(incoming.x+18,incoming.y);rule='intercept_rest_window'
                    else:
                        guard_active=True;rule='draw_intruder_away'
                        target=(min(1540,a.x+30),a.y)
                        d=math.dist((a.x,a.y),(incoming.x,incoming.y))
                        if d<65 and a.energy>a.max_energy/5+8:speed=a.sprint_speed
                        if d>150:target=(a.x,a.y)
...
def main():
    ...
    for c in configs:
        ...
        (OUT/f'protocol-{args.suite}-v1.json').write_text(json.dumps(dict(scope=__doc__,runs=rows),indent=2,default=lambda v:v.item())+'\n')
