...
def run(seed=1, mode='site', wall=True, seconds=180, native_predators=True,
        stress=False, ripeness_wait=15., record=None):
    ...
    if record:
        ...
        recorder = ReplayRecorder(env, title=f'Observation wall deployment / {mode} / seed {seed}',
            policy='wall_deployment.controller.WallPolicy', seed=seed, every=20,
            scenario='generated' if mode=='generated' else 'arranged natural site', native_render=True,
    ...
    if recorder:
        ...
        recorder.save(record,reason='horizon' if env.agents else 'extinction',overwrite=True)
    return result
...
def main():
    ...
    policy_sha = hashlib.sha256(Path(__file__).with_name('controller.py').read_bytes()).hexdigest()
    for seed in map(int,args.seeds.split(',')):
        for wall in ([True,False] if args.control=='paired' else [args.control=='wall']):
            record = OUT/f'{Path(args.output).stem}-seed{seed}-{"wall" if wall else "colony"}.json.gz' if args.record else None
            row = run(seed=seed,mode=args.mode,wall=wall,seconds=args.seconds,
                ...
                ripeness_wait=args.ripeness_wait,record=record)
            runs.append(row)
