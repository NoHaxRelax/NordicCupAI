...
def build(output, include_all=False):
    if include_all:
        ...
        entries = json.loads((ROOT/'recordings/manifest.json').read_text())['recordings']
        sources = [(entry, (ROOT/entry['file']).resolve()) for entry in entries]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--all', action='store_true', help='Include every discovered replay (can create a very large file). Default: compact curated demonstrations.')
