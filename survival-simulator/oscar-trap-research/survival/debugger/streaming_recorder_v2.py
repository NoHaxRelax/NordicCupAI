...
class EventJournal(FrameJournal):
    def __init__(self, directory):
    ...
    def __getitem__(self,index):
        if isinstance(index,slice) and index.start is not None and index.start>=self.recent_start:
            end=None if index.stop is None else index.stop-self.recent_start
            return self.recent[index.start-self.recent_start:end:index.step]
...
class ReplayRecorder(InMemoryRecorder):
    def __init__(self, *args, spool_dir=None, **kwargs):
        ...
        self.events = EventJournal(directory)
        self.meta['frame_storage'] = 'disk journals for frames and events; all captured records preserved'
    ...
    def save(self, path, *, reason='recording stopped', overwrite=False):
        ...
        try:
            ...
            self.events.stream.close()
            self.events.path.unlink()
