...
def inspect_replay(data):
    ...
    return dict(id=replay_digest(payload), title=meta.get('title') or 'Untitled run',
                ...
                renderer='native' if any(f.get('native_image') for f in frames) else 'state',
...
class ReplayCatalog:
    ...
    def refresh(self):
        with self.scan_lock:
            ...
            ordered = sorted(groups.values(), key=lambda row: (row['order'], row['entry']['group'], row['entry']['title'], row['entry']['source']))
