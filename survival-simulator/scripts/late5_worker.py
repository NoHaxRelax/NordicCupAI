...
def run(job):
 ...
 for name,cfg in job['configs'].items():
  game_start=time.monotonic();r=one((name,job['seed'],cfg));r['game_seconds']=time.monotonic()-game_start;rows.append(r)
 return dict(id=job['id'],rows=rows,seconds=time.monotonic()-start)
def main():
 files=list((ROOT/'fastsim').glob('*.hpp'))+list((ROOT/'fastsim').glob('*.cpp'))+[ROOT/'models/self_stuck.hpp',ROOT/'models/late_activation.hpp',ROOT/'scripts/late5_config.py',ROOT/'scripts/late5_worker.py',ROOT/'scripts/evaluate_frozen1000.py']
