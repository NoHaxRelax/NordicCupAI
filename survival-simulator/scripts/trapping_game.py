...
From repo root: python survival-simulator/run.py trapping
...
def main():
    ...
    parser.add_argument('--policy-seed', type=int, default=0,
                        help='Independent controller RNG seed; never use the world seed as a policy input')
    parser.add_argument('--seconds', type=float, default=3000)
    parser.add_argument('--corner-pockets', action='store_true', help='Opt in to observed corner-pocket trap candidates')
    ...
    policy = EntrapmentPolicy(seed=args.policy_seed, include_corner_pockets=args.corner_pockets)
    env = core.env
    ...
    sources = [*sorted((ROOT/'models').rglob('*.py')), *sorted((ROOT/'models'/'exploration'/'config').glob('*.json')),
               *sorted((ROOT/'src').rglob('*.py')), Path(__file__)]
    atom(folder/'manifest.json', dict(seed=args.seed, policy_seed=args.policy_seed, corner_pockets=args.corner_pockets,
        horizon=args.seconds, dt=core.dt, policy_class='models.core.EntrapmentPolicy',
