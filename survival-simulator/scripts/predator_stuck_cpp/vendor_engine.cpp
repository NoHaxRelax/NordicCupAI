...
// Evaluator-only additive accounting. Never used by physics, observations or RNG.
struct DiagnosticEvent { int kind; int64_t id, other; double a, b, c; };

// Read-only predator instrumentation. These values never drive engine decisions.
enum PredatorDebugField { D_X, D_Y, D_HEADING, D_ENERGY, D_REST, D_BIOME,
    D_MODE, D_EDGE_COUNT, D_LOCAL_OBSTACLES, D_REQUESTED_DISTANCE, D_RELATIVE_DIRECTION,
    D_HAS_DIRECTION, D_TURN, D_SCALED_DISTANCE, D_ABSOLUTE_DIRECTION,
    D_ATTEMPTS, D_ACCEPTED, D_REJECTED_MASK, D_FIRST_BLOCKER,
    D_EDGE_X1, D_EDGE_Y1, D_EDGE_X2, D_EDGE_Y2, D_CLOSEST_X, D_CLOSEST_Y,
    D_EDGE_CLEARANCE, D_EDGE_ANGLE, D_BOUNDS_CLAMPED, D_COUNT };
using PredatorDebug = std::array<double, D_COUNT>;
...
public:
    ...
    bool predator_debug_enabled = false;
    std::vector<PredatorDebug> predator_debug;
    PredatorDebug* active_predator_debug = nullptr;
    void debug_attempt(int index, bool blocked, double x, double y, double size,
                       const std::vector<int32_t>& local) {
        if (!active_predator_debug) return;
        auto& d = *active_predator_debug;
        d[D_ATTEMPTS] += 1;
        if (!blocked) { d[D_ACCEPTED] = index; return; }
        d[D_REJECTED_MASK] += std::ldexp(1.0, index);
        if (d[D_FIRST_BLOCKER] >= 0) return;
        for (auto id : local) {
            const auto& o = obstacles[id];
            if (o.x-size < x && x < o.x+o.w+size && o.y-size < y && y < o.y+o.h+size) {
                d[D_FIRST_BLOCKER] = id; break;
            }
        }
    }
    void diagnostic(int kind, int64_t id, int64_t other=-1, double a=0, double b=0, double c=0) {
        if (diagnostics_enabled) diagnostics.push_back({kind, id, other, a, b, c});
    }
    ...
    void spawn_child(const Creature& parent) {
        ...
        add_agent(a);
        diagnostic(4, agents.back().id, parent.id, agents.back().energy);
    }
    void kill_agent_at(size_t idx, int cause, int64_t predator_key=-1) {
    ...
    void update_entity_position(Creature& c, double distance, bool has_dir, double direction,
        ...
        if (active_predator_debug) {
            (*active_predator_debug)[D_SCALED_DISTANCE] = distance;
            (*active_predator_debug)[D_ABSOLUTE_DIRECTION] = direction;
        }
        double prev_x = c.x, prev_y = c.y;
        double new_x = prev_x + distance * np_cos(direction);
        double new_y = prev_y + distance * np_sin(direction);
        bool initially_blocked = in_obstacle(new_x, new_y, c.size, local_obs);
        debug_attempt(0, initially_blocked, new_x, new_y, c.size, local_obs);
        if (initially_blocked) {
            ...
            for (int i = 0; i < max_attempts; i++) {
                ...
                bool blocked = in_obstacle(tx, ty, c.size, local_obs);
                debug_attempt(i+1, blocked, tx, ty, c.size, local_obs);
                if (!blocked) { c.x = tx; c.y = ty; break; }
            }
        } else {
            c.x = new_x; c.y = new_y;
        }
        double unclamped_x = c.x, unclamped_y = c.y;
        keep_in_bounds(c);
        if (active_predator_debug)
            (*active_predator_debug)[D_BOUNDS_CLAMPED] = (c.x != unclamped_x || c.y != unclamped_y);
    }
    ...
    void agent_step(const Action& act) {
        ...
        if (idx < 0) return;
        {
            ...
            double energy_before = a.energy, x_before = a.x, y_before = a.y;
            ...
            diagnostic(0, a.id, -1, energy_before-a.energy, std::max(0., act.move_distance),
                       std::hypot(a.x-x_before, a.y-y_before));
            agents_dirty = true;
            energy_before = a.energy;
            update_entity_direction(a, act.turn_angle);
            diagnostic(1, a.id, -1, energy_before-a.energy);
        }
        if (act.spawn && agents[idx].energy > 100) {
            Creature parent = agents[idx];
            spawn_child(parent);
            agents[idx].energy -= 100;
            diagnostic(2, agents[idx].id, -1, 100.);
        }
    }
    ...
    void non_agent_step() {
        ...
        while (i < agents.size()) {
            Creature& a = agents[i];
            double energy_before = a.energy;
            a.age += dt;
            a.energy -= dt * BIOMES[biome_at(a.x, a.y)].energy_drain_rate;
            if (a.energy <= 0) {
                diagnostic(3, a.id, -1, energy_before-a.energy);
            ...
            if (fr_set.used) {
                ...
                for (size_t r = keys_buf.size(); r-- > 0;) {
                    ...
                    if (distance < ag.size + fr.radius) {
                        double before_meal = ag.energy;
                        double e = ag.energy + fr.energy;
                        ag.energy = (e < ag.max_energy) ? e : ag.max_energy;
                        diagnostic(5, ag.id, fr.fruit_id, fr.energy, ag.energy-before_meal, fr.age);
        ...
        // ---- predators
        if (predator_debug_enabled) predator_debug.resize(predators.size());
        for (size_t p = 0; p < predators.size(); p++) {
            active_predator_debug = predator_debug_enabled ? &predator_debug[p] : nullptr;
            if (active_predator_debug) {
                auto& d = *active_predator_debug; d.fill(0);
                const auto& pr = predators[p];
                d[D_X]=pr.x; d[D_Y]=pr.y; d[D_HEADING]=pr.direction;
                d[D_ENERGY]=pr.energy; d[D_REST]=pr.resting; d[D_BIOME]=biome_at(pr.x,pr.y);
                d[D_ACCEPTED]=-1; d[D_FIRST_BLOCKER]=-1;
            }
            {
            ...
            if (!keys_buf.empty()) {
                ...
                for (size_t q = touching.size(); q-- > 0;) {
                    ...
                    score -= ae / 100;
                    kill_agent_at((size_t)ai, 1, pr.key);
                }
            }
            if (pr.energy <= 0) pr.resting = true;
        }
        active_predator_debug = nullptr;
        ...
        while (f < fruits.size()) {
            Fruit& fr = fruits[f];
            if (fr.age > 100) {
                diagnostic(7, fr.fruit_id, -1, fr.energy);
    ...
    void predator_act(Creature& pr, const std::vector<Obs>& obs, const std::vector<int32_t>& lobs) {
        if (active_predator_debug) {
            (*active_predator_debug)[D_LOCAL_OBSTACLES] = lobs.size();
            for (const auto& o : obs) if (o.type == 4) (*active_predator_debug)[D_EDGE_COUNT] += 1;
        }
        ...
        if (closest) {
            if (active_predator_debug) (*active_predator_debug)[D_MODE] = 3;
            ...
            if (active_predator_debug) (*active_predator_debug)[D_MODE] = 1;
            ...
            for (const Obs& o : obs) {
                ...
                if (first || h < best_d) {
                    best_d = h; best_x = cx; best_y = cy; first = false;
                    if (active_predator_debug)
                        for (int k=0;k<4;k++) (*active_predator_debug)[D_EDGE_X1+k] = o.c[k];
                }
            }
            ...
            if (active_predator_debug) {
                auto& d=*active_predator_debug;
                d[D_CLOSEST_X]=best_x; d[D_CLOSEST_Y]=best_y;
                d[D_EDGE_CLEARANCE]=best_d-pr.size; d[D_EDGE_ANGLE]=ang;
            }
            do_turn = true; turn = ta;
            do_move = true; move_dist = pr.speed; move_has_dir = true; move_dir = ta;
        } else {
            if (active_predator_debug) (*active_predator_debug)[D_MODE] = 2;
        ...
        if (do_move) {
            if (active_predator_debug) {
                auto& d=*active_predator_debug;
                d[D_REQUESTED_DISTANCE]=move_dist; d[D_RELATIVE_DIRECTION]=move_dir;
                d[D_HAS_DIRECTION]=move_has_dir; d[D_TURN]=turn;
            }
...
PyObject* Engine_diagnostics(EngineObject* self, PyObject* arg) {
    int enabled = PyObject_IsTrue(arg);
    if (enabled < 0) return nullptr;
    self->eng->diagnostics_enabled = enabled;
    self->eng->diagnostics.clear();
    Py_RETURN_NONE;
}
PyObject* Engine_pop_diagnostics(EngineObject* self, PyObject*) {
    const auto& rows = self->eng->diagnostics;
    PyObject* list = PyList_New((Py_ssize_t)rows.size());
    for (size_t i=0; i<rows.size(); ++i) {
        const auto& r = rows[i];
        PyList_SET_ITEM(list, (Py_ssize_t)i, Py_BuildValue("(iLLddd)", r.kind,
            (long long)r.id, (long long)r.other, r.a, r.b, r.c));
    }
    self->eng->diagnostics.clear();
    return list;
}
PyObject* Engine_predator_ids(EngineObject* self, PyObject*) {
    PyObject* list = PyList_New((Py_ssize_t)self->eng->predators.size());
    for (size_t i=0; i<self->eng->predators.size(); ++i)
        PyList_SET_ITEM(list, (Py_ssize_t)i, PyLong_FromLong(self->eng->predators[i].key));
    return list;
}
...
PyMethodDef Engine_methods[] = {
    ...
    {"diagnostics", (PyCFunction)Engine_diagnostics, METH_O, "enable evaluator-only accounting"},
    {"pop_diagnostics", (PyCFunction)Engine_pop_diagnostics, METH_NOARGS, "drain native per-step accounting"},
    {"predator_ids", (PyCFunction)Engine_predator_ids, METH_NOARGS, "stable evaluator-only predator keys"},
...
PyModuleDef moduledef = {PyModuleDef_HEAD_INIT, "_engine", "Native survival simulator engine", -1, module_methods};
