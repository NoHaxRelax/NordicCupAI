...
async def main(a):
 ...
 def trial(i,it):
  ...
  if path.exists():data=json.loads(path.read_text())
   ...
   prior=trials[i];rng=np.random.default_rng(46000+i*1000+it)
   x,method=(initial(i),'late preset')if it==1 else propose([t['x']for t in prior],[t['objective']for t in prior],rng,len(SPACES[i]))
   data=dict(x=list(x),method=method,config=config(i,x),iteration=it);save(path,data)
 ...
 def update():
  ...
  for i,name in enumerate(NAMES):
   while len(trials[i])<ITERATIONS:
    ...
    runtime=float(np.mean([completed[k]['rows'][0]['game_seconds']for k in ids]));score=float(np.mean(scores));eligible=runtime<20.
    feasible=[t['score']for t in trials[i]if t['eligible']]+([score]if eligible else [])
    data.update(score=score,mean_game_seconds=runtime,eligible=eligible,objective=score if eligible else -1000.-100.*(runtime-20.),best=max(feasible)if feasible else None)
    trials[i].append(data);save(out/f'{name}-trials.json',trials[i])
    print(json.dumps(dict(family=name,iteration=it,mean=data['score'],best=data['best'],mean_game_seconds=runtime,eligible=eligible)),flush=True)
    if it<ITERATIONS:trial(i,it+1)
  if all(len(t)==ITERATIONS for t in trials) and stage=='training':
   winners={name:max([x for x in t if x['eligible']],key=lambda x:x['score'])for name,t in zip(NAMES,trials)if any(x['eligible']for x in t)}
   save(out/'excluded-families.json',[name for name in NAMES if name not in winners])
   save(out/'frozen-winners.json',winners) # persisted BEFORE any held-out game
   stage='final'
   configs={name:w['config']for name,w in winners.items()};configs['expanded_food_baseline']=BASE
   # All eligible models plus baseline on a given map execute on one worker/CPU. Rotate order.
   for seed in TEST:
  ...
  save(out/'progress.json',dict(stage=stage,trials={n:len(t)for n,t in zip(NAMES,trials)},completed_jobs=len(completed),completed_games=sum(len(r['rows'])for r in completed.values()),queued=queue.qsize(),elapsed_session=time.monotonic()-start,updated=time.time()))
 if a.pilot:
 ...
 async def connection(p):
  while not done.is_set():
   inflight={};proc=None
   try:
    cmd=f'cd {REMOTE}/survival-simulator && /workspace/lucas-families10/.venv/bin/python -u scripts/late5_worker.py'
