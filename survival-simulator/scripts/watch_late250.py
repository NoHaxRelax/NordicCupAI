...
REMOTE = r'''
import json,pathlib
r=pathlib.Path('/workspace/lucas-late250')
i=INDEX
...
if (e/'games.jsonl').exists():
    ...
    phase=f'final evaluation {games}/2200 games'
...
def main():
    ...
    args=ap.parse_args()
    pods=json.loads((ROOT/'docs/late250/dispatch.json').read_text())
    try:
        while True:
            ...
            print(f"BO: {sum(s.get('trials',0) for s in statuses)}/640 trials | "
                  f"Final: {sum(s.get('games',0) for s in statuses)}/22000 games",flush=True)
            if args.once:break
