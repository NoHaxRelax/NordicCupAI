...
def public_input(value):
    """Serialize observed search inputs; rebuild geometry caches when loading."""
    caches = {'_steering_geometry', '_forecast_pred_geometry', '_following_geometry',
              '_handoff_visibility_walls', '_route_planner', '_lane_rejoin_planner'}
    if isinstance(value, dict):
        return {str(k): public_input(v) for k,v in value.items() if k not in caches}
    if isinstance(value, (list, tuple)):
        return [public_input(v) for v in value]
    if hasattr(value, 'tolist'):
        return public_input(value.tolist())
    if hasattr(value, 'item'):
        return public_input(value.item())
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    raise TypeError(f'Unrecognized public input type: {type(value).__name__}')
...
def main():
    ...
    parser.add_argument('--time',type=float,
                        help='Capture an earlier guide tick instead of its recorded fatal tick')
    parser.add_argument('--out',type=Path,required=True)
    parser.add_argument('--public-input-out',type=Path,
                        help='Save ordinary search inputs for fast counterfactual iterations')
    ...
    for file in sorted((args.folder/'chunks').glob('*.json.gz')):
        ...
        for row in rows:
            ...
            if abs(row['time']-target_time)<1e-6:
                ...
                old_debug=track.memory.get('debug')
                original_forecast=old_debug.get('forecast') if isinstance(old_debug,dict) else None
                ...
                bait=local(pose,policy.site['goal'])
                edges=[(local(pose,a),local(pose,b)) for a,b in track.edges]
                if args.public_input_out:
                    fixture=dict(seed=manifest['seed'],agent=args.agent,time=row['time'],
                        recorded_action=original.model_dump(),bait=public_input(bait),
                        edges=public_input(edges),state=public_input(state),
                        target_observation=public_input(track.observers.get(args.agent)),
                        memory=public_input(track.memory),
                        note='Ordinary DTO and observed-map search inputs; geometry caches omitted. '
                             'The recorded action has already passed the original steering search.')
                    args.public_input_out.parent.mkdir(parents=True,exist_ok=True)
                    args.public_input_out.write_text(json.dumps(fixture,indent=2,allow_nan=False)+'\n')
                guide_steering.search=replacement.search
                changed=guide_steering.prioritize(original.model_dump(),bait,edges,state,track.memory,
                ...
                result=dict(seed=manifest['seed'],agent=args.agent,time=row['time'],
                    ...
                    original_forecast=original_forecast,changed_forecast=track.memory['debug']['forecast'],
            ...
            sim.pop_events()
    raise RuntimeError('Requested tick not present in recording')
