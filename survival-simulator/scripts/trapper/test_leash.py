...
def run_case(seed, n_obstacles, staffed, second, seconds, verbose=False):
...
if __name__ == '__main__':
    ...
    if a.verbose_seed is not None:
        r = run_case(a.verbose_seed, a.obstacles, a.staffed == 1 if a.staffed >= 0 else a.verbose_seed % 2 == 0, bool(a.second), a.seconds, verbose=True)
        print(r); sys.exit(0)
    ok = 0; fails = []
    for seed in range(a.start, a.start + a.cases):
        staffed = (seed % 2 == 0) if a.staffed < 0 else bool(a.staffed)
        r = run_case(seed, a.obstacles, staffed, bool(a.second), a.seconds)
        ok += r['ok']
