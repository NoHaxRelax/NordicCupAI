...
import subprocess
import argparse
p=argparse.ArgumentParser();p.add_argument('--avx2',action='store_true');args=p.parse_args()
...
out=build/('scan-avx2' if args.avx2 else 'scan')
subprocess.run(['g++','-O3','-std=c++17',*(['-mavx2'] if args.avx2 else []),'-I',str(build),str(ROOT/'models/seed_shadow/scan.cpp'),'-o',str(out)],check=True)
print(out)
