...
REMOTE = r'''
import json,pathlib
r=pathlib.Path('/workspace/lucas-sharedfood20')
i=INDEX
...
if (v/'complete.json').exists():phase='checkpoint validation complete; awaiting full games'
if (e/'games.jsonl').exists():
    with (e/'games.jsonl').open() as f:games=sum(1 for line in f if line.endswith('\n'))
    phase=f'final evaluation {games}/2400 games'
if (e/'complete.json').exists():phase='final evaluation complete'
errors=[]
for log in ('run.log','validation.log','final.log'):
    p=r/log
    if p.exists():
...
def check(p, key):
    ...
    try:
        ...
        return dict(phase='SSH timeout or malformed response; job state unknown')
...
def main():
    ...
    args=ap.parse_args()
    pods=json.loads((ROOT/'docs/sharedfood20/dispatch.json').read_text())
    try:
        while True:
