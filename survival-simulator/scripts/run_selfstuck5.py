...
REMOTE='/workspace/lucas-selfstuck5'
...
async def main(a):
 ...
 def trial(i,it):
  ...
  if path.exists():data=json.loads(path.read_text())
   ...
   prior=trials[i];rng=np.random.default_rng(26000+i*1000+it)
   x,method=(initial(i),'center')if it==1 else propose([t['x']for t in prior],[t['score']for t in prior],rng,len(SPACES[i]))
 ...
 def update():
  ...
  if all(len(t)==ITERATIONS for t in trials) and stage=='training':
   ...
   # All six models on a given map execute on one worker/CPU. Rotate order.
   for seed in TEST:
 ...
 async def connection(p):
  while not done.is_set():
   inflight={};proc=None
   try:
    cmd=f'cd {REMOTE}/survival-simulator && /workspace/lucas-families10/.venv/bin/python -u scripts/selfstuck_worker.py'
