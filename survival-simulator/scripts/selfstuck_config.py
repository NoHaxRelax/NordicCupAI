...
NAMES=['boundary_steer','rock_face_steer','rock_corner_escape','preserve_stationary','adaptive_stuck','gentle_migration']
...
GENTLE=json.loads((ROOT/'vendor/gentle-migration/gentle-config.json').read_text())['config']
SPACES.append({'trap_bias':(1.,40.),'trap_protect':(0.,6.),'trap_reach':(80.,260.),'trap_active_r':(45.,160.)})
GENTLE_DEFAULT={**GENTLE,'trap_reach':180.}
def config(i,x):
 if i==5:return {**GENTLE,**{k:float(lo+(hi-lo)*v)for(k,(lo,hi)),v in zip(SPACES[i].items(),x)}}
 return {**BASE,'stuck_mode':i+1,**DEFAULT,**{k:float(lo+(hi-lo)*v) for (k,(lo,hi)),v in zip(SPACES[i].items(),x)}}
def initial(i):return [((GENTLE_DEFAULT if i==5 else DEFAULT)[k]-lo)/(hi-lo) for k,(lo,hi) in SPACES[i].items()]
