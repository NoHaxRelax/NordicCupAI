...
def _find_slots(rects, width, height, cfg, kinds) -> List[TrapSite]:
    ...
    sites: List[TrapSite] = []
    for comp, bounds in enumerate(find_objects(components), 1):
        if bounds is None:
            continue
        # Sort only the pocket's bounding box, not the whole world once per
        # pocket. This matters when new observed rocks trigger an online scan.
        local_clearance, local_rock = clearance[bounds], to_rock[bounds]
        mask = (components[bounds] == comp) & usable[bounds]
        if not mask.any():
            continue
        ...
        scores = np.where(mask & (local_clearance <= SLOT_HOLD_MAX),
                          np.floor(local_clearance) * 1000.0 + local_rock, -1.0)
        if not (scores > 0).any():
            scores = np.where(mask, np.floor(local_clearance) * 1000.0 + local_rock, -1.0)
        ...
        for idx in tries:
            bx, by = np.unravel_index(int(idx), scores.shape)
            bx, by = bx + bounds[0].start, by + bounds[1].start
