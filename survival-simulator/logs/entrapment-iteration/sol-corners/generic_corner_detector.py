...
def detect(static):
    ...
    for region in _parts(safe,.25):
        ...
        # Rear must be agent-reachable, outside the main predator component,
        # opposite the handoff, and its complete segment must remain at least
        # contact distance from the nominal held/front centre. This last check
        # prevents reversing front and rear in diagonal pockets.
        handoff=rear=None
        for trial in sorted(handoffs,key=lambda q:q.distance(front_route.boundary),reverse=True):
            vx,vy=bait.x-trial.x,bait.y-trial.y;n=math.hypot(vx,vy) or 1.
            for distance in (70.,55.,40.,30.,25.):
                for angle in (0.,.2,-.2,.4,-.4,.7,-.7):
                    c,s=math.cos(angle),math.sin(angle);ux=(vx*c-vy*s)/n;uy=(vx*s+vy*c)/n
                    q=Point(bait.x+ux*distance,bait.y+uy*distance);segment=LineString([bait,q])
                    if (agent.covers(q) and agent.covers(segment) and not front_component.covers(q)
                            and trial.distance(segment)>=15.05):
                        handoff,rear=trial,q;break
                if rear:break
            if rear:break
