...
def _guide(bait, edges, agent, context, memory):
    ...
    if predator is None or not_following:
        memory.pop('_pace_move_cap', None)
    ...
    if final_approach:
        ...
        memory.pop('_pace_sprinting', None)
        memory.pop('_pace_move_cap', None)
    else:
        pacing = memory.get('_pace_sprinting', False)
        tick = context.get('tick', 0)
        previous = memory.get('_pace_observation')
        closing = 0.
        if previous is not None and tick > previous['tick']:
            closing = max(0., (previous['distance']-predator['distance'])/(tick-previous['tick']))
        # Observations precede the native predator move. Conservatively reserve
        # one maximum 15-unit predator step plus the measured closing trend.
        predicted_clearance = predator['distance'] - 15. - closing
        if predicted_clearance <= 55.: pacing = True
        elif predator['distance'] >= 70. and predicted_clearance > 55.: pacing = False
        memory['_pace_observation'] = dict(tick=tick, distance=predator['distance'])
        memory['_pace_predicted_clearance'] = predicted_clearance
        memory['_pace_sprinting'] = pacing
        speed = agent['sprint_speed'] if pacing else agent['speed']
        memory['_pace_move_cap'] = speed
    ...
    memory['debug'] = {
        ...
        'following_check': memory['_following_debug'],
        'pace_sprinting': memory.get('_pace_sprinting'),
        'energy': round(agent.get('energy', 0.), 2),
        'energy_below_20_percent': agent.get('energy', 0.) < agent.get('max_energy', 500.)/5,
        'pace_predicted_clearance': memory.get('_pace_predicted_clearance'),
    }
