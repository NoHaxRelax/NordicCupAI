...
bool parse_params(PyObject* d, orchard::Params& P) {
    ...
    F fs[] = {{"evade_search", &P.evade_search}, {"evade_energy", &P.evade_energy}, {"evade_goal", &P.evade_goal}, {"pred_cone", &P.pred_cone},{"corner_start_angle", &P.corner_start_angle}, {"corner_gaze", &P.corner_gaze}, {"corner_mode", &P.corner_mode}, {"corner_tolerance", &P.corner_tolerance}, {"corner_budget", &P.corner_budget},{"cap_mult", &P.cap_mult}, {"cap_min", &P.cap_min}, {"cap_max", &P.cap_max}, {"n0", &P.n0},
...
PyObject* Engine_corner_debug(EngineObject* self, PyObject*) {
    if(!self->pol)Py_RETURN_NONE;
    PyObject* agents=PyList_New(0);
    self->pol->minds.each([&](const int64_t& aid,orchard::MindP& m){if(m->corner_phase){
        PyObject* row=Py_BuildValue("(Lidddddddddd)",(long long)aid,m->corner_phase,m->corner_pred.x,m->corner_pred.y,m->corner_heading,m->corner_want,m->corner_error,m->pose->p.x,m->pose->p.y,m->pose->theta,m->corner_goal.x,m->corner_goal.y);
        PyList_Append(agents,row);Py_DECREF(row);
    }});
    return Py_BuildValue("{s:L,s:L,s:L,s:L,s:N}","attempts",(long long)self->pol->corner_started,"aligned",(long long)self->pol->corner_aligned,"missed",(long long)self->pol->corner_missed,"aborted",(long long)self->pol->corner_aborted,"agents",agents);
}
...
PyMethodDef Engine_methods[] = {
    ...
    {"corner_debug", (PyCFunction)Engine_corner_debug, METH_NOARGS, "Observed corner steering counters and poses"},
