...
struct Params {
    double cap_mult = 0.3, cap_min = 4, cap_max = 20, n0 = 80., tree_half = 600., tree_slots = 1,
           breed_reserve = 200., emergency_reserve = 105., ripen_wait = 15., sweep_rate = 0.03,
    ...
    // Harvest support uses public observations only. Disable for historical parity.
    bool harvest_ready = true;
};
...
public:
    ...
    bool ready(const FruitM& f, double energy = OINF, bool old = false) const {
        // Initial fruit already supplies positive energy. During establishment,
        // collect it to feed the reserve instead of waiting twenty seconds to ripen.
        if (P.harvest_ready && time < 60.) return true;
    ...
    const std::vector<Act>& call(const AState* sts, size_t nsts, double sim_time) {
        ...
        if (P.harvest_ready && time <= 0.5 && pop >= 3 && pop < 8) {
            // Six survivors plus two harvest participants, established before the
            // first predator. Do not repeatedly drain parents later in the game.
            int64_t need = 8 - pop - (int64_t)spawn_set.size();
            for (const AState& s : states) {
                if (need <= 0) break;
                if (spawn_set.count(s.aid) || s.biome == RIVER) continue;
                const Plan& pl = plans[s.aid];
                if (s.energy - cost_now(pl.dist, pl.turn, s) <= 125.) continue;
                spawn_set.insert(s.aid);
                --need;
            }
        }
