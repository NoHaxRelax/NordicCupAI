...
// Public-state entry point: allows the same compiled policy to run against the
// Python simulator and to receive observation filtering without touching engine
// state. The engine owned by this object is not read by this method.
PyObject* PE_policy_observe(PolicyEngineObject* self, PyObject* args) {
    PyObject* states; double now;
    if (!PyArg_ParseTuple(args, "Od", &states, &now)) return nullptr;
    if (!self->pol) { PyErr_SetString(PyExc_RuntimeError, "policy_init first"); return nullptr; }
    if (!PyList_Check(states)) { PyErr_SetString(PyExc_TypeError, "states must be a list"); return nullptr; }
    size_t n = PyList_GET_SIZE(states);
    std::vector<polabi::AState> buf(n);
    std::vector<std::vector<polabi::Obs>> observations(n);
    auto number = [](PyObject* d, const char* key, double fallback) {
        PyObject* v = PyDict_GetItemString(d, key);
        return v ? PyFloat_AsDouble(v) : fallback;
    };
    for (size_t i = 0; i < n; ++i) {
        PyObject* d = PyList_GET_ITEM(states, i);
        if (!PyDict_Check(d)) { PyErr_SetString(PyExc_TypeError, "state must be a dict"); return nullptr; }
        auto& s = buf[i];
        s.aid = (int64_t)number(d, "agent_id", 0);
        s.energy = number(d, "energy", 0); s.age = number(d, "age", 0);
        s.speed = number(d, "speed", 10); s.sprint = number(d, "sprint_speed", 20);
        s.hear = number(d, "hearing_radius", 60); s.cone = number(d, "vision_angle", PI / 3);
        s.vr = number(d, "vision_range", 200); s.max_energy = number(d, "max_energy", 400);
        s.biome = polabi::GRASSLAND;
        PyObject* biome = PyDict_GetItemString(d, "biome");
        if (biome) for (int b = 0; b < 5; ++b)
            if (PyObject_RichCompareBool(biome, s_biome_names[b], Py_EQ) == 1) s.biome = b;
        PyObject* obs = PyDict_GetItemString(d, "observations");
        if (!obs || !PyList_Check(obs)) { PyErr_SetString(PyExc_TypeError, "observations must be a list"); return nullptr; }
        for (Py_ssize_t j = 0; j < PyList_GET_SIZE(obs); ++j) {
            PyObject* o = PyList_GET_ITEM(obs, j);
            if (!PyDict_Check(o)) { PyErr_SetString(PyExc_TypeError, "observation must be a dict"); return nullptr; }
            PyObject* tag = PyDict_GetItemString(o, "type");
            if (!tag || !PyUnicode_Check(tag)) continue;
            const char* t = PyUnicode_AsUTF8(tag);
            polabi::Obs v{};
            v.type = !strcmp(t, "Fruit") ? 0 : !strcmp(t, "Agent") ? 1 :
                     !strcmp(t, "Predator") ? 2 : !strcmp(t, "Tree") ? 3 : !strcmp(t, "Edge") ? 4 : -1;
            if (v.type < 0) continue;
            if (v.type == 4) {
                PyObject* coords = PyDict_GetItemString(o, "coords");
                if (!coords) { PyErr_SetString(PyExc_ValueError, "edge needs coords"); return nullptr; }
                for (int k = 0; k < 2; ++k) {
                    PyObject* p = PySequence_GetItem(coords, k);
                    if (!p) return nullptr;
                    for (int axis = 0; axis < 2; ++axis) {
                        PyObject* value = PySequence_GetItem(p, axis);
                        if (!value) { Py_DECREF(p); return nullptr; }
                        v.c[k * 2 + axis] = PyFloat_AsDouble(value); Py_DECREF(value);
                    }
                    Py_DECREF(p);
                }
            } else {
                v.distance = number(o, "distance", 0); v.angle = number(o, "angle", 0);
                v.has_rel_dir = PyDict_GetItemString(o, "rel_dir") != nullptr;
                v.rel_dir = number(o, "rel_dir", 0);
                v.has_id = PyDict_GetItemString(o, "id") != nullptr;
                v.id = (int64_t)number(o, "id", 0);
            }
            observations[i].push_back(v);
        }
        s.obs = &observations[i];
    }
    if (PyErr_Occurred()) return nullptr;
    return acts_to_py(self->pol->call(buf.data(), buf.size(), now));
}
...
PyMethodDef policy_extra_methods[] = {
    {"policy_observe", (PyCFunction)PE_policy_observe, METH_VARARGS,
     "decide from public state dictionaries and simulation time only"},
