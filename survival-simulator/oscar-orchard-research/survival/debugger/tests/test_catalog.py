...
class Discovery(unittest.TestCase):
    ...
    def test_newest_recordings_are_first(self):
        write(self.root/'results/older.json', fixture('Older', '2026-09-17T09:00:00Z'))
        write(self.root/'results/newest.json', fixture('Newest', '2026-09-17T12:00:00+00:00'))
        write(self.root/'results/middle.json', fixture('Middle', '2026-09-17T11:00:00Z'))
        write(self.root/'results/unknown.json', fixture('Unknown', 'not-a-timestamp'))
        result = self.catalog.refresh()
        self.assertEqual(
            [row['title'] for row in result['recordings']],
            ['Newest', 'Middle', 'Older', 'Unknown'],
        )
