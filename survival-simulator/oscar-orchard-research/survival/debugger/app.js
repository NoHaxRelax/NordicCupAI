...
(() => {
  ...
  const options = { trails: true, vision: true, hearing: true, labels: false };
  let toastTimer;
  ...
  async function openReplay(blob, name = 'Local replay', sampleFile = '', requestedLoadId = null) {
    ...
    try {
      ...
      const subtitle = [state.nativeMode ? 'Organizer renderer' : 'State-only fallback', replay.meta.policy && `Policy: ${replay.meta.policy}`, replay.meta.seed != null && `Seed ${replay.meta.seed}`, replay.meta.record_interval != null && `${number(replay.meta.record_interval, 3)} s nominal capture`].filter(Boolean);
    ...
    for (const item of state.manifest) {
      ...
      if (item.group) {
        if (currentGroupName !== item.group) {
          ...
          currentGroup.label = String(item.group);
  ...
  async function loadManifest(initial = false) {
    ...
    try {
      ...
      const signature = JSON.stringify(state.manifest.map(item => [item.file, item.label, item.title, item.description, item.source, item.group, item.renderer]));
      ...
      if (initial && state.manifest.length && !state.replay && !state.loadId) {
        const preferred = state.manifest.find(item => item.renderer === 'native') || state.manifest[0];
        await loadSample(preferred.file);
      }
    } catch (error) {
  ...
  function renderInspector() {
    ...
    if (!selected) {
      $('perception-note').textContent = state.nativeMode
        ? 'Hearing and obstacle-clipped vision are baked into the organizer rendering'
        : 'State-only fallback · nominal hearing and vision shown for every creature';
      return;
    }
    ...
    $('perception-note').textContent = state.nativeMode ? 'Hearing and obstacle-clipped vision are baked into the organizer rendering' : 'State-only fallback · nominal hearing and vision shown for every creature';
  }
  ...
  function drawWorld() {
    ...
    if (!state.nativeMode) {
      for (const entity of frame.predators) drawSenses(entity, 'predators');
      for (const entity of frame.agents) drawSenses(entity, 'agents');
