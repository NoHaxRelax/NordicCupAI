...
C++ port of `vendor/survival-simulator` (commit acfc31a) exposed as a CPython
...
Research harness: `SURVIVAL_ENGINE=fast python research/orchard/harness_np.py ...`
(also works for `sweep.py`, `opt.py` and `research/society/harness.py`). Result JSON
is unchanged apart from an added `"engine"` field. `--record` still needs the
Python engine.
