...
class ExpertPolicy:
    ...
    def decide(self, inputs: PolicyInputs) -> ActionRequest:
        ...
        if in_danger:
            ...
            # Every other movement mode below avoids observed edges; fleeing must
            # too, or an escape bearing that points at a wall sprints straight into
            # it every tick instead of deflecting, losing the race to a predator
            # that closes in via open ground.
            if inputs.obstacle_edges:
                direction, distance = avoid_edges(
                    direction, distance, inputs.obstacle_edges,
                    cfg.movement.sprint_clearance_margin, inputs.agent_id,
                )
