...
def _guide(bait, edges, agent, context, memory):
    ...
    stationary = memory['_following_debug'].get('status') == 'stationary_or_resting'
    memory['_stationary_ticks'] = memory.get('_stationary_ticks', 0)+1 if stationary else 0
    if (stationary and memory['_stationary_ticks'] <= 40
            and predator['distance'] >= 60. and math.hypot(*bait) > HEARING_TARGET):
        # Observed lack of motion, not hidden predator energy/rest state.
        # Avoid spending energy running away from a stationary follower and
        # doubling back later. Bound the wait so blocked predators get a new angle.
        memory['debug'] = dict(mode='wait_for_observed_motion',
                               stationary_ticks=memory['_stationary_ticks'],
