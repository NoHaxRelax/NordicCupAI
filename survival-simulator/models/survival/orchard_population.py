...
Vendored unmodified from survival-simulator/oscar-orchard-population at a7a7d634
(oscar-orchard-research/survival/research/orchard/orchard.py). This is a later
revision than models/survival/oscar_orchard.py and is the one whose parameter
surface matches the tuned configs in that branch's configs/best-configs.json;
the older file silently swallows unknown keys through **_, so tuned settings
would no-op against it. Carries no predator handling at all - see
models/orchard_evasion_policy.py for the evasion layer built on top.
