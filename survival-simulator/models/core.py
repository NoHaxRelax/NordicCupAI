...
class EntrapmentPolicy:
    def __init__(self, seed=0, *, include_corner_pockets=False):
        self.include_corner_pockets = include_corner_pockets
        config = load_config()
        ...
        self.orchard = OrchardPolicy(seed=seed, extra_old=False, heir_age=1e9)
        self.site = None
    ...
    def _candidate_sites(self, static):
        sites = our_sites(static)
        keep_corner = self.site is not None and self.site.get('site_kind') == 'corner_pocket'
        if self.include_corner_pockets or not sites or keep_corner:
            sites += our_sites(static, corner_only=True)
        return sites
    ...
    def _find_site(self):
        ...
        for gid, group in self.estimator.groups.items():
            ...
            sites = self._candidate_sites(static)
    ...
    def _select_bait(self, states, excluded):
        ...
        for aid, s in states.items():
            ...
            if aid in excluded or pose.group_id != self.site_group or pose.uncertainty > 8.: continue
            ...
            choices.append((travel+5 < deadline, old, s['age'] if old else 0., life-travel, -distance, aid, travel))
    ...
    def _bait_roles(self, states):
        ...
        if self.bait not in states: self.bait = None
        if self.incoming not in states: self.incoming = None
        ...
        # A rolling replacement is dispatched as soon as bait is established.
        # It joins the goal, then both stay; no last-second energy gamble.
        if self.incoming is None:
            ...
            due = self.bait is None or (selected is not None and active_life < selected[1]+20.)
            if due:
                self.incoming = None if selected is None else selected[0]
    ...
    def _track_predators(self, states):
        ...
        for track in self.tracks.values():
            ...
            if (track.guide_id in states and track.held_since is not None
                ...
                assigned.discard(track.guide_id)
                track.guide_id = None
                track.memory = {}
    ...
    def _guide_action(self, track, s):
        ...
        context = dict(tick=round(self.now*10), time=self.now, dt=.1,
                       ...
                       target_predator=target, associate_target=True)
    ...
    def _shared_predators(self, aid):
        """Fresh teammate observations transformed through the estimated shared frame."""
        pose = self.estimator.poses.get(aid)
    ...
    def _trace(self, aid, reason, action):
        if not getattr(self, 'record_decisions', False):
            return
        value = {key:getattr(action, key) for key in ('move_distance', 'move_direction', 'turn_angle', 'spawn_agent')}
        history = self.decision_trace.setdefault(aid, [])
        if not history or history[-1]['action'] != value:
            history.append(dict(reason=reason, action=value))
    ...
    def __call__(self, states_list, sim_time):
        ...
        for aid, s in states.items():
            ...
            if role in ('bait', 'replacement_bait', 'retired_bait'):
                ...
                action, avoiding = avoid_predators(action, s, bait_local, shared_predators=self._shared_predators(aid))
                if avoiding:
                    role = 'avoiding_predator'
                self.roles[aid] = role
            self._trace(aid, role, action)
            # Reproduction belongs to fit young gatherers/explorers, not bait.
            if role not in ('explorer', 'gatherer') or self.orchard.minds[aid].old or s['age'] >= 55.:
