...
class DeliveryTests(unittest.TestCase):
    def act(self, handoff, memory, observed=True, bait=(30.,0.)):
        ...
        # These isolate delivery after target association. Newcomer selection is
        # covered separately; a fresh sighting inside the held crowd is rejected.
        target = agent['observations'][0] if observed else None
        return guide(bait, EDGES, agent, dict(tick=0, dt=.1, handoff=handoff, target_predator=target), memory)
