...
case $cmd in
  ...
  deploy) for p in "$@"; do
      tar -C $SRC/.. -czf - nightsim/_nengine.cpp nightsim/_npolicy.hpp nightsim/__init__.py nightsim/build.py nightsim/run.py nightsim/escape.py nightsim/trapsite.py \
  ...
  launch) p=$1; job=$2; args=$3; envs=${4:-}
    sshp $p "cd /workspace/night/code/survival || exit 1; setsid nohup env $envs $(venv $p)/bin/python nightsim/run.py $args --out /workspace/night/runs/$job.jsonl > /workspace/night/runs/$job.log 2>&1 < /dev/null & echo launched $job on $p" ;;
  pull) p=$1; job=$2
