...
def one(job):
    ...
    sx, sy = sinks[si][0], sinks[si][1]
    base = dict(label=label, seed=seed, sink=si, sink_n=sinks[si][2], sink_drop=sinks[si][3] if len(sinks[si]) > 3 else None, dg=dg, dp=dp, bear=bear, speed=sp)
    ...
    found = None
    for k in range(60):
        a = rng.uniform(0, 2 * math.pi) if k else math.atan2(600 - sy, 800 - sx)
        gx, gy = sx + dg * math.cos(a), sy + dg * math.sin(a)
    ...
    eng.dbg_true_poses(); eng.dbg_sg_sinks([(s[0], s[1]) for s in sinks])
    for _ in range(2):
...
if __name__ == '__main__':
    ...
    ap.add_argument('--minn', type=int, default=3); ap.add_argument('--top', type=int, default=3)
    ...
    for l, kw in cfgs.items():
        for s in parse_seeds(a.seeds):
            ...
            ns = min(a.top, sum(1 for x in SINKS[s] if x[2] >= a.minn))
            for si in range(ns):
                for dg in a.dg:
                    for dp in a.dp:
                        for b in a.bear:
