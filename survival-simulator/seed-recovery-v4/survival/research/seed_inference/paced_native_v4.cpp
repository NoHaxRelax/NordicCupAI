// Native live validation experiment: C++ HTTP, policy, search and model loop.
...
struct Game:std::enable_shared_from_this<Game>{
    ...
    bool searching=false,stopped=false,model_failed=false;int64_t recovered=-1,latest_seq=-1;J latest;
    ...
    void model_loop(std::unique_ptr<Engine> e,uint32_t seed){
        ...
        try{while(true){
            ...
            advance(*e,packet["actions"]);replay.capture(*e,packet["actions"]);
    ...
    void search_loop(){
        ...
        while(seconds(begin)<1800){
            ...
            if(found){
                ...
                if(match){std::lock_guard<std::mutex>g(mu);recovered=seed;changed.notify_all();save_json(dir/"recovered.json",{{"seed",seed},{"worker",worker},{"search_seconds",seconds(begin)},{"sim_time",last_time},{"recovered_ns",now_ns()}});auto self=shared_from_this();std::thread([self,seed,e=std::move(e)]()mutable{self->model_loop(std::move(e),seed);}).detach();}
            }
    ...
    J predict(const J& body,int64_t received_ns){
        ...
        {std::lock_guard<std::mutex> guard(mu);received=Clock::now();last_time=body["sim_time"];add_samples(samples,freshness.update(body).pose_input);seq=packets.size();
            J packet={{"seq",seq},{"received_ns",received_ns},{"before",body},{"actions",actions}};packets.push_back(packet);packet_log<<packet.dump()<<'\n';packet_log.flush();
        ...
        {std::unique_lock<std::mutex> guard(mu);
            if(!model_failed&&latest_seq>=int64_t(seq)-8&&latest_seq>=0)changed.wait_for(guard,std::chrono::milliseconds(1500),[&]{return latest_seq>=int64_t(seq)||model_failed;});
            bool committed=!model_failed&&latest_seq==int64_t(seq);forecast_responses+=committed;
