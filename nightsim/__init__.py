...
try:
    from . import _nengine as _engine
except ImportError as exc:  # pragma: no cover
    raise ImportError('nightsim._nengine is not built; run: python nightsim/build.py') from exc
...
class FastEnv:
    ...
    @property
    def predators(self):
        return [SimpleNamespace(x=x, y=y, direction=d, energy=e, resting=r, size=10)
                for (x, y, d, e, r) in self._e.predators()]
